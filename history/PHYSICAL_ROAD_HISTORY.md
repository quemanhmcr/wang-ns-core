# Wang–NS Physical-Road History

> **Historical snapshot.** This file was formerly `history/README.md`. Statements such as “exactly eight tracked documents” describe the physical-road distillation at that time, not the current repository layout. The current history map is [README.md](README.md). The current whole-state spectral-signature core is [../core/spectral_signature/README.md](../core/spectral_signature/README.md).


An eight-document distillation of the Wang–Navier–Stokes physical-road exhaustion programme as it stood in this historical phase.
...
## Historical repository invariant

At that historical snapshot, the distilled physical-road set intentionally contained exactly **eight tracked documents**:

1. `README.md` (now preserved as `PHYSICAL_ROAD_HISTORY.md`) — map, status legend, historical purification, reading order.
...
Target line budgets: `README <= 210`, `CONTROL_VOLUME_METHOD <= 100`, `PHYSICAL_CORE <= 700`, `MIXED_FRONTIER <= 520`, `SOLUTION_MAP <= 180`, `BSVO_FULL_STATE_FRONTIER <= 1300`, `NS_POLAR_COMPATIBILITY_ARCHITECTURE <= 1900`, `NEO_ANCHOR_COMPILER <= 900`.
