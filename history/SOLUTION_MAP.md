...
It must show, using only true NS identities, that one fixed positive critical-hinge level `K_rad(R_kappa,t)=kappa` cannot make unbounded UV excursions.  The observer hierarchy is now provably exhausted, and the late Theory-2 reconstruction sharpens the final mechanism further.  The complete moving flag supplies the historical missing half derivative before contraction.  Reciprocal Lemma A has an exact rational certificate and Lemma B gives finite companion multiplicity, so the old aligned reciprocal-incidence seam is closed.  Equal-heat diamond geometry leaves exactly the collision invariants `1,k_x,k_y,k_z,|k|^2`: the first four are amplitude/translation modes and the fifth is heat.

The final open target is therefore not another front, norm, clock, wallet, or stand-alone Codazzi budget.  It is a **hypocoercive Polar--Korn coupling** between reciprocal A+B conductance/finite incidence plus projective/reflection tangent control and the distinguished normal generator `-nu C^2`, using the exact mother/signature Korn observability and the radial--Jordan--Codazzi acceleration identity to retain the built-in `-W_Lambda^2` square.  A modified energy estimate of this type would make one compulsory visibility-speed carrier time-integrable and contradict the exact singular-endpoint requirement `int W_Lambda^2/U = infinity`.  No such modified-energy estimate is proved yet.  See `core/spectral_signature/HALF_DERIVATIVE_POLAR_KORN_BRIDGE.md`.
