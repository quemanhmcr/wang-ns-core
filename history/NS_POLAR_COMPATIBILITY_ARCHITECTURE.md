...
\]
On the divergence-free state space, \(Pu=u\) and \(C^2=-\Delta\). The canonical spectral readings
\[
H=\operatorname{sgn}C,\qquad \Lambda=|C|
\]
are derived from the same curl anchor, with
\[
\boxed{H^2=P,\qquad C=H\Lambda,\qquad \Lambda^2=C^2.}
\]
Motion is generated by the single transport derivation
\[
\boxed{\delta_uX=[D_u,X],\qquad D_u=u\cdot\nabla,}
\]
and its parabolic-covariant completion. In particular the mother first jet is
\[
\boxed{E_u=[D_u,C],}
\]
while support/Hodge, angular and radial first jets are functional-calculus renderings of this same parent after localization. Any proposed pressure, torsion, stress, Codazzi, measure, gauge or new rate must first be tested for exact parentage under projection, contraction, functional calculus or covariant prolongation.
...
## 100. Strategic reset: NEO Singularity Normal Form, not trajectory surveillance — CANDIDATE PROGRAM / RESEARCH RULE
The latest reconstruction changes the role NEO should play in the regularity problem. The implicit task used through much of the earlier exploration was stronger than the theorem requires: reconstruct and control the entire sequence of Navier--Stokes metamorphoses along an arbitrary trajectory. That programme was useful for discovering the compiler, but it is not logically necessary for excluding finite-time blow-up and it risks an endless descendant loop.

The distinction is now explicit:
\[
\boxed{\text{reconstruction completeness}\ \neq\ \text{control completeness}.}
\]
NEO has accumulated strong evidence for the first statement: local differential faces, canonical isotropic spectral readers, Hodge/pressure faces, hard crossing, regeneration, finite time jets, variational descendants, order-two spectral normal forms and the critical-contact compiler all return to the anchors
\[
\boxed{u,\ P,\ C,\ C^2,\ t.}
\]
Nothing in that statement by itself supplies the critical analytic estimate required for global regularity. A language may reconstruct a dynamical system without proving every trajectory stable.

The blow-up problem asks for much less than a classification of all turbulence:
\[
\boxed{\text{exclude the existence of a finite-time singular object}.}
\]
Accordingly, NEO should be used where exact reconstruction is strongest: on a scale-normalized object extracted from a hypothetical singular endpoint, not as a surveillance system that must follow every transient costume of the raw solution.

### 100.1 Exact covariance of the terminal microscope — EXACT
For a parabolic zoom
\[
v_n(y,s)=r_n\,u(x_n+r_n y,\ t_n+r_n^2s),
\qquad r_n\downarrow0,
\]
the genetic equation retains its form:
\[
\boxed{(v_n)_s=P[X_{v_n},C]v_n-\nu C^2v_n.}
\]
Thus every NEO identity that is legitimate at the corresponding regularity and localization scale has the same parentage after translation and NS scaling. A singular tangent/ancient limit, if an external compactness theorem provides one with enough convergence, cannot acquire a hidden mechanism outside the compiler merely because the original trajectory was complicated.

### 100.2 Division of labour — CANDIDATE PROGRAM
The proposed proof architecture has three layers and they should not be conflated.

1. **Analytic extraction, outside the NEO ontology.** From a hypothetical finite singular endpoint, construct a nonzero normalized tangent or ancient object with the compactness/tightness and convergence needed to pass the relevant NEO expressions to the limit. Profile decomposition, concentration compactness, local compactness, suitable-solution theory or unique continuation belong here if needed. NEO is not required to regenerate compactness itself.
2. **NEO terminal compiler.** Run a finite normalization pass on the extracted object: quotient the affine Euler-invisible span \(1,C\); apply the already-compiled spectral bend, contact, curl-null and square-anchor normalizers; and output a finite list of exact terminal alternatives. The compiler is used to turn approximate degeneracy of the prelimit sequence into exact algebraic identities of the limiting object.
3. **Rigidity / Liouville closure.** Prove that no nonzero object satisfying the extraction contract can inhabit any terminal normal form. This is the point where finite-energy \(\mathbb R^3\) rigidity, spatial/frequency tightness, boundary-leakage estimates or unique continuation should act.

Schematically,
\[
\boxed{
\text{hypothetical blow-up}
\to\text{normalized singular profile}
\to\text{finite NEO normal forms}
\to\text{rigidity contradiction}.
}
\]
This is intentionally different from
\[
\text{raw trajectory}\to\text{defect}\to\text{next defect}\to\text{next jet}\to\cdots .
\]

### 100.3 Why limits are the natural habitat of NEO rigidity — DEDUCTION
Many of the strongest NEO statements are exact zero-set classifications rather than generic coercive estimates. Examples already generated by the compiler include:

- affine critical bend \(A_u u=0\), where \(|C|\) is Euler-invisible modulo \(1,C\);
- finite spectral closure of a next bend jet, which forces support onto finitely many signed-curl levels and is incompatible with a nonzero \(L^2(\mathbb R^3)\) Fourier field when exact;
- curl-null physical states, which are trivial in the finite-energy whole-space setting;
- pure-helicity / shifted-hinge boundary states, where normal escape is an exact positive square;
- second-order local heat contact, where continuum multiplicity is reduced by the NEO contact normalizer to normal escape, finitely many projective directions, a curl-null seam, or an explicit square-anchor heat branch;
- the heat branch, whose local identity already yields a boundary-leakage estimate rather than a new mechanism.

Along an arbitrary raw trajectory these objects may only be approximately degenerate, forcing difficult quantitative assembly questions. A compact singular sequence can instead convert a normalized defect tending to zero into an exact defect of the limit. The strategic hope is therefore
\[
\boxed{\text{compactness converts approximate NEO geometry into exact NEO rigidity}.}
\]
This is a candidate proof principle, not yet an extraction theorem.

### 100.4 Current candidate terminal forms — NOT YET AN EXHAUSTIVE THEOREM
The existing compiler suggests that a scale-normalized singular object should be tested first against the following already-generated forms:
\[
\boxed{
\begin{array}{ll}
\text{A.}&\text{affine spectral / zero-bend},\\
\text{B.}&\text{curl-null},\\
\text{C.}&\text{finite signed-curl spectral closure},\\
\text{D.}&\text{square-anchor heat-locked / boundary-leakage},\\
\text{E.}&\text{finite local critical-contact geometry}.
\end{array}}
\]
This list is a research normalizer, not a proved exhaustive classification. Its purpose is to force the next theorem to reduce the number of admissible singular profiles rather than to generate another diagnostic vocabulary.

A particularly promising interaction is that the global spectral compiler turns failure of bend-direction escape into concentration near finitely many signed-curl levels, while the local contact compiler turns failure of transverse heat contact into finitely many projective directions unless curl-null or \(C^2\) becomes explicit. On a compact tangent object, the dangerous residual geometry is therefore expected to be finite in both spectral level and local direction. The genetic convolution \(P[X_u,C]u\) must then preserve that finite structure or create an escape already typed by NEO. Whether this yields a finite algebraic classification is **OPEN**.

### 100.5 NEO Terminality Principle — CANDIDATE PRINCIPLE
\[
\boxed{
\begin{gathered}
\text{Regularity does not require controlling every admissible NS metamorphosis.}\\
\text{It is enough to classify the scale-normalized metamorphosis that can survive}\\
\text{at a hypothetical singular endpoint. Because all finite NS costumes found so far}\\
\text{compile from the same anchors, the tangent object has no hidden mechanism outside NEO.}\\
\text{The endpoint should therefore be attacked as a finite normal-form and rigidity problem,}\\
\text{not as a mechanism-by-mechanism control problem along the whole trajectory.}
\end{gathered}}
\]

### 100.6 Anti-loop protocol from this point onward — RESEARCH RULE
The ontology is frozen unless a genuine counterexample forces an extension. A newly compiled descendant counts as progress only if it does at least one of the following:

1. eliminates an admissible singular normal form;
2. lowers a real derivative/integrability gap;
3. produces coercivity whose owner is already finite under the physical NS budgets;
4. gives a direct contradiction with the extraction contract.

If a calculation merely says that the remaining freedom lives in the next NEO jet, record the parentage and stop. Formal closure already predicts that result. The next central task is therefore **not another repair jet**. It is to formulate a rigorous singular-profile extraction contract strong enough to pass the finite terminal compiler, then prove a NEO rigidity theorem for the resulting tangent/ancient objects.

**OPEN.** No singular-profile extraction theorem, exhaustive terminal classification, or global regularity theorem is asserted here. The strategic claim is narrower: the reconstruction programme has matured enough that further progress should be measured by elimination of singular normal forms, not by the number of additional NS costumes successfully regenerated.
