...
**EXACT FULL-STATE `B/S/V/O` FRONTIER:** write `N=-P(u.grad u)=u_t+nu Lambda^2u` and `G=curl N=S omega-(u.grad)omega`.  Critical action, Piola pressure reflection and the complex silent-gate law remain exact.  The newer full-polarization pair Pythagoras separates helical-reader cancellation from actual geometric-pair silence: unequal-radius silence is common-normal shear, while equal-radius pairs possess a wider phase-null manifold.  Nevertheless every nondegenerate closed geometric triad with all three Fourier coefficients active and nonzero full internal interaction has a reality-companion pair acceleration at radius strictly larger than its largest root.  A closed one-triad finite-Fourier state is therefore nonlinear-silent, but the terminal family includes planar shear and monochromatic `2D3C` states as well as Beltrami.  At a fixed child, direct viscosity organizes cancellation by `kappa=|p|^2+|q|^2`; Laplace uniqueness forces passive heat-orbit cancellation to close fiberwise.  Actual pair sources obey `dot F_e=(r_p+r_q)F_e`, so unequal-heat persistent cancellation requires genuine parent radial work, whereas same-heat cancellation can remain phase-driven.  The exact Riccati law `dot(n/a)=(F_N/a)-(n/a)^2` returns all non-affine repair to the genuine `F_N/G` dynamics.  Under an explicit global affine-synchronization hypothesis the solution collapses to regular monochromatic translation/decay (trivial in finite-energy `R^3`); the unresolved block is the same-heat non-affine repaired continuum cancellation needed for `Y=>bot`.  See `BSVO_FULL_STATE_FRONTIER.md`.
