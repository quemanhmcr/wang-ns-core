...
**DISTILLED EXACT PRESSURE-FREE VORTICITY / BIOT--SAVART LAW:** pressure is not an independent `H^1` road.  Modewise `S_hat:H0_hat=0`, while `partial_t S=sym grad N+nu Delta S`; more fundamentally `u=curl(-Delta)^-1 omega` and NS is equivalent to `partial_t omega+(u.grad)omega=(omega.grad)u+nu Delta omega`, with pressure reconstructed afterward from `-Delta p=|S|^2-|omega|^2/2`.  Biot--Savart gives `S(x)=(3/8pi)PV int[(h cross omega(x+h)) tensor h+h tensor(h cross omega(x+h))]/|h|^5 dh`; for `xi=omega/|omega|`, longitudinal strain reads only the pure `l=2` angular kernel `(xi.n)(xi cross n)`.  After global symmetrization, `Q=int omega.S omega=(3/8pi)int int[((omega_y-omega_x).h)((omega_x cross omega_y).h)]/|h|^5 dxdy`: exact stretching requires longitudinal vorticity increment and genuine spatial vortex-pair chirality in the signed integrand.  This is the same scalar as `(C_F/2)int Z_tri`, not a triad-to-pair coupling or chirality currency.  Along the physical flow, `d(F^-1 omega(X,t))/dt=nu F^-1 Delta omega`, `det F=1`; pressure does not enter the Cauchy invariant.
**EXACT EDGE-SOURCE/LAMB DIAGNOSTIC:** for the derived pre-Hahn edge source `f_e`, modal NS gives its exact damped product equation and `G_(-1,edge)<=4C_F^2E||N||_2^2`, `N=-P(u.grad u)=P(u x omega)`.  The resulting `S2_edge,K2_edge` are exact **source-level quadratic diagnostics**, not kinetic-energy/work/dissipation currencies or causal owners.  At first contact, the interval `J` and split `K<>Lambda M` are proof readers; the Cauchy lower bound on `K2_edge` is an **EXACT analytic estimate**, not a no-estimate distilled law.  Rotational NS gives `<N,-Delta u>=int omega.S omega`, while the vorticity-only/Biot--Savart theorem above removes pressure from the dynamical state entirely.  Finite `int_0^(T*)||N||_2^2dt` would bound `||grad u||_2`, hence the Kenig--Koch endpoint norm, so Lamb divergence is not a second mechanism road.  Static radial/helicity/cancellation typing and pressure/same-field typing are closed.  The remaining nonredundant target is the exact compatibility of late UV first contacts with Biot--Savart two-point vortex geometry, material deformation and viscosity; the signed two-point integrand, Lamb, `J,Lambda,S2_edge,K2_edge`, Cauchy, coarea and Hodge/entropy objects remain representations/readers, never new owners or currencies.
