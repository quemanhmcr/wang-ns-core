...
**DISTILLED EXACT RADIAL/HELICITY TRAFFIC:** homochiral triads create zero `Hdot^(1/2)` stock; positive minority work increases every increasing-convex radial moment.  More generally, for radii `a<p<q` with low recipient `T_a>0`, the two invariants exhaust four classes.  Homochiral and low-minority classes have the middle root as unique donor and `A_up>=A_down` (equality only homochiral); homochiral circulation still creates enstrophy `(p-a)(q-a)T_a`.  If the minority helicity is at `p` or `q`, both high roots are donors, `A_up=0`, and `A_down=-sum rho_iT_i` is exact critical-moment loss; the enstrophy moment is negative too.  Thus inward radial action is never an untyped “reuse”: it is same-event circulation/forward companion or true moment loss.  No finite reset follows from convex moments because every nonconstant increasing-convex radial observable is UV-unbounded.
...
**DISTILLED EXACT EDGE-SOURCE/LAMB ACTION:** edge retuning obeys `G_(-1,edge)<=4C_F^2E||N||_2^2`, `N=-P(u.grad u)=P(u x omega)`, so first-contact heat action reduces to the actual Leray-Lamb forcing.  Rotational NS gives the orthogonal physical split `u x omega=N+grad B`, `B=p+|u|^2/2`, hence `||u x omega||_2^2=||N||_2^2+||grad B||_2^2`; pressure is the gradient complement, not another nonlinear supplier.  Also `<u,N>=0` while `<N,-Delta u>=int omega.S omega`: Lamb forcing is tangent to kinetic energy and its `H^1` radial projection is vortex stretching.  Do not identify Lamb action with positive stretching: under the exact symmetry `u->-u`, `N` and `||N||_2^2` are unchanged while `int omega.S omega` changes sign.  A finite singular endpoint still requires `A_down=infinity` or infinite Lamb `L_t^2L_x^2` action; the remaining frontier is rigidity inside these genuine circulation/moment-loss and Lamb/stretching routes.  Hodge/entropy remains downstream.
