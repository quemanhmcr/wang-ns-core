...
**DISTILLED EXACT PRESSURE-TIDAL/VORTICITY-FRAME LAW:** put `g=-Delta p=|S|^2-|omega|^2/2` and `H0=Hess p+(g/3)I`.  For every nonzero Fourier output `z`, `H0hat(z)=(I/3-zhat tensor zhat) ghat(z)`: all pressure-pair tensors feeding one output lie on this single trace-free tensor line, so their cancellation is scalar, not a matrix-angle mechanism.  The actual same-field source is `ghat(z)=-C_F int (z.uhat(x))(z.uhat(z-x))dx`; one unordered physical pair has bare scalar `g_(x,y)=-2(z.u_x)(z.u_y)` and helical geometric magnitude `|xxy|^2|a_xa_y|/(|x||y|)`, independent of helicity signs.  In physical space `H0=(4pi)^-1 PV int_0^infinity dr/r int_(S^2)(3 n tensor n-I)g(x-rn)dOmega`, so only the `l=2` quadrupole of the same NS Poisson source survives.  At `omega!=0`, with `xi=omega/|omega|`, `P=I-xi tensor xi`, `a=xi.Sxi`, `b=P S xi`, `D=P S P+(a/2)P`, and `tau=xi.H0 xi`, exact NS gives `S=a xi xi+xi b+b xi-(a/2)P+D`, `|S|^2=3a^2/2+2|b|^2+|D|^2`, and `D_t a=-a^2/2+5|b|^2/3+|D|^2/3-|omega|^2/6-tau+nu[2b.Deltaomega/|omega|+xi.(Delta S)xi]`.  `tau` is a physical tidal readout, never a work/owner/currency.
**EXACT EDGE-SOURCE/LAMB DIAGNOSTIC:** for the derived pre-Hahn edge source `f_e`, modal NS gives its exact damped product equation and `G_(-1,edge)<=4C_F^2E||N||_2^2`, `N=-P(u.grad u)=P(u x omega)`.  The resulting `S2_edge,K2_edge` are exact **source-level quadratic diagnostics**, not kinetic-energy/work/dissipation currencies or causal owners.  At first contact, the interval `J` and split `K<>Lambda M` are proof readers; the Cauchy lower bound on `K2_edge` is an **EXACT analytic estimate**, not a no-estimate distilled law.  Rotational NS itself gives `u x omega=N+grad B`, `B=p+|u|^2/2`, `<u,N>=0`, and `<N,-Delta u>=int omega.S omega`.  The instantaneous algebraic involution `u->-u` fixes `N` but reverses the stretching functional, so Lamb action cannot be renamed positive stretching.  Finally, finite `int_0^(T*)||N||_2^2dt` would bound `||grad u||_2` by the gradient NS identity; energy plus `||u||_Hdot^(1/2)^2<=||u||_2||grad u||_2` would then bound the Kenig--Koch endpoint norm.  Hence every finite singular endpoint already requires `int_0^(T*)||N||_2^2dt=infinity`; therefore `A_down=infinity OR Lamb=infinity` is not a new mechanism dichotomy.  The static radial/helicity/cancellation typing is closed by the signed-curl path law and Betchov bridge, and pressure-Hessian typing is closed by the scalar fixed-output/quadrupole/vorticity-frame laws above.  The remaining nonredundant target is the same-field compatibility of the late first-contact `a,b,D,omega` geometry with the `l=2` quadrupole generated by that very `u`; Hodge/entropy remains downstream.
