...
**EXACT FULL-STATE `B/S/V/O` FRONTIER — MOTHER-LAW FORM:** the dedicated frontier now starts from `N=u_t+nu Lambda^2u`.  Modewise and on every fixed Fourier/helicity control volume, actual nonlinear work is exactly the action defect of the true `N/u_t/nu Lambda^2u` force triangle.  Pushing the same work to signed curl `x=s|k|` gives the hinge potential `Psi(a)` and the quadratic action profile `A(a)=2nu Psi(a)`, with profile height/slope/curvature equal to shifted critical work / actual cut flux / actual modal work and `Q` equal to profile area.  On smooth finite-energy `R^3`, positive critical production forces a single-helicity tail with zero nonlinear kinetic-energy source but positive helicity/critical transfer.  On that tail, if `U=Pu`, `mu=<Lambda U,U>/||U||^2`, `V=(Lambda-mu)U`, `S=||V||^2`, the true nonlinear acceleration splits exactly as `PN=[H/(2S)]V+N_perp`, with `N_perp` orthogonal to both `U` and `Lambda U`; all first-moment radial hardening is therefore carried by one forced sorter while phase/polarization/same-heat complexity is isolated in the moment-orthogonal remainder.  Viscosity acts with the opposite sign on the same radial-shape direction, and work-curvature / neutral-front motion are governed by the genuine `F_N/G` field.  Full-pair Pythagoras, reality-companion leakage, heat-fiber Laplace rigidity and conditional affine synchronization remain exact microscopic guards.  The unresolved theorem is to exclude repeated unbounded-scale regeneration of the forced neutral-tail sorter by the true `N_perp/F_N` dynamics; equivalently `Y=>bot`.  See `BSVO_FULL_STATE_FRONTIER.md`.
