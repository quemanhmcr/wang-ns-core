...
**DISTILLED EXACT MINORITY CHANNEL:** homochiral triads create zero `Hdot^(1/2)` stock; heterochiral production feeds the `+/-` helicity critical sectors equally.  For positive minority work, the smaller majority root is the unique donor and the larger majority root is a simultaneous positive recipient with upward radial action `(a+p)T_m>aT_m`.  Thus minority production with smaller parent `p>=Ra` is bounded by `V_rho/(R+1)`; deep HH->low creation belongs to the existing radial-variation/viscosity route.  The minority edge itself remains canonical `B` with `r_e<=99/100`.

**EXACT DIAMOND/SPIN LEAKAGE:** every same-helicity parent atom has `F_h=-Lambda e^(2ih phi)F_-h`, `Lambda>1`, so minority-source hiding must also cancel a larger majority-helicity spin-2 companion.  A physical same-output diamond additionally has mandatory cross pairs at `q+-d`.  Unequal-scale nonparallel helical parents satisfy `||F_nm||>=(1/2)|sin theta||M^2-N^2|/|n+m||a_na_m|`, giving `||F_+||||F_-||>=kappa_diamond sigma_1sigma_2` for the two cross outputs.  Hence strong same-output cancellation leaks state-source; at zero tolerance every active off-diagonal diamond has at least one nonzero source among its four non-original parent pairs.  Equal-radius/collinear faces can only make the quantitative constant small, not create an exact silent active diamond.  No causal Hahn is introduced.

**DISTILLED EXACT NULL/HEAT STABILITY:** for same-helicity `v_h`, intrinsic Beltrami and shear defects `B=inf_K||(curl-hK)v_h||_2`, `S=inf_e[int(1-(k.e)^2/|k|^2)|vhat_h|^2]^(1/2)` control every surviving modal source by `C_F||v_h||_2B` and `2C_FKmax||v_h||_2S`; both zero sets are nonlinear-null.  Same-output source atoms carry physical heat rate `kappa=|p|^2+|q-p|^2`; without actual nonlinear parent forcing, cancellation persisting on a time interval must be fiberwise in `kappa` by Laplace uniqueness.  **OPEN:** obtain the quantitative measure-level cover inside those heat-rate fibers / parent-forced retuning.  Hodge/entropy remains downstream only.
