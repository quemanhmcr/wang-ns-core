...
**EXACT FIRST-CONTACT / TWO-POINT ANTI-BRIDGE:** canonical UV inflow does not choose stretching sign.  On the homochiral spectrum `1<sqrt(2)<sqrt(3)` at cut `6/5`, one can choose `Z_A<0` so the crossing `1->sqrt(2)` has any prescribed mass `m`; after dilation by `sqrt(2/3)`, choose `Z_B>0` so `2/sqrt(3)->sqrt(2)` crosses the same cut with the same mass, and rotate to the same child wavevector/helicity.  Nonzero Waleffe coupling plus amplitude/phase scaling realizes both.  Hence no atomwise first-contact->`Q+`/positive-chirality map exists.  Yet at a finite singular endpoint, for every `t_0<T_*`, `sup_(t_0<t<T_*) int_(t_0)^t Q=+infinity`; by volume preservation this is the same cumulative signed material-pair integral after every late contact.  The only remaining nonredundant target is an exact spacetime correlation between late radial traffic and the material segment-curvature/viscous dynamics above; Lamb, `J,Lambda,S2_edge,K2_edge`, Cauchy/coarea/Hodge and the positive part of the pair integrand remain readers/diagnostics, never owners or currencies.
