...
**DISTILLED EXACT NULL/HEAT STABILITY:** intrinsic Beltrami/shear defects control surviving same-helicity source, and viscosity sorts persistent unforced cancellation fiberwise by `kappa`.  On each `RP^2` heat sphere, projective disintegration gives the exact law `S^2-||F||^2=S int(1-||b||^2)d lambda+(1/2)int int||b-b'||^2d lambda d lambda'`: within-line cancellation is the already-routed reflection rectangle; between-line cancellation is a positive projective diamond-incidence law.  **OPEN:** build the fixed-`q` codimension-one coarea/pushforward bridge, including the multiplicity created when `q` is integrated, to the primitive 6D NS parent-pair source, or route the only alternative—actual parent nonlinear retuning.  Never treat the collision measure as source/work currency; Hodge/entropy remains downstream.
