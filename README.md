...
**EXACT VECTOR HEAT-SPHERE LAW:** at fixed child `q` and parent helicity, `r=2p-q` turns equal heat rate into the physical unordered sphere `S^2/(r~-r)=RP^2`.  The unordered Leray vector-orbit norm is `|q.r||qxr|sqrt((P+M)^2+Q^2)|a_pa_m|/[4PMQ(P+M)]` and the continuum density multiplies it by common `C_F`; its only geometric zeros are the equal-radius Beltrami equator and collinear shear pole.  Minority/spin-two laws are merely its helical coordinates.  For `dF=xi d sigma`, `S=sigma(Omega)`, vector polarization gives `S^2-||int dF||^2=(1/2)int int||xi-xi'||^2`; off-diagonal pairs are physical diamonds with mandatory extra full-NS pair sources.  No causal Hahn is introduced.

**DISTILLED EXACT NULL/HEAT STABILITY:** intrinsic Beltrami/shear defects control every surviving same-helicity modal source by `C_F||v_h||_2B` and `2C_FKmax||v_h||_2S`.  Source atoms carry `kappa=|p|^2+|q-p|^2`; without actual nonlinear parent forcing, cancellation persisting on a time interval is Laplace-unique and therefore fiberwise on one heat sphere.  **OPEN:** obtain the quantitative vector-measure no-hiding theorem on those `RP^2` fibers, or show that the necessary parent-forced retuning enters an already-controlled NS state/work law.  Hodge/entropy remains downstream only.
