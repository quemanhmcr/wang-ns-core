...
**DISTILLED EXACT MINORITY CHANNEL:** homochiral triads create zero `Hdot^(1/2)` stock; heterochiral production feeds the `+/-` critical sectors equally.  Positive minority work has unique donor `p` and two real recipients `a,q`; their normalized mean radius is strictly larger than `p`.  Hence the actual donor row increases every increasing-convex radial moment (all `rho^alpha`, `alpha>=1`) while deep HH->low creation is already bounded by radial variation.  This spectral order is not a finite budget: every nonconstant increasing convex radial observable is UV-unbounded.  The minority edge remains canonical `B`, `r_e<=99/100`.
...
**DISTILLED EXACT NULL/HEAT/CANCELLATION STABILITY:** intrinsic Beltrami/shear defects control surviving same-helicity source, while viscosity sorts persistent unforced cancellation fiberwise by `kappa`.  The unordered orbit-source square `S2=int dz int|C_F S_pair|^2d lambda_z^unord` obeys `S2<=2C_F^2E||grad u||_2^2` and `int_0^T S2dt<=C_F^2E_*^2/nu`.  On a heat fiber of separation radius `R`, the unordered base mass is exactly `pi R/4`; hence if `S,F,L2` are source variation, resultant and source-square density, `4(S^2-||F||^2)/(pi R)<=L2`.  For actual positive/negative child work `P,N`, Hilbert projection gives the sharper `4PN/(pi R|u_q|^2)<=L2`; if `F=0`, then `P=N`.  Thus persistent unforced hiding pays canonical pair-source-square fiberwise.  Equal-heat coarea still identifies where diamond leakage goes, but is no longer needed to manufacture a cancellation currency.  **OPEN:** rule out finite-time accumulation of these parabolically summable `S2`/parent-forcing payments.  Hodge/entropy remains downstream.
