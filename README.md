...
**DISTILLED EXACT EDGE-SOURCE/LAMB ACTION:** for `f_e=C_ea_xa_y`, `(partial_t+nu kappa_e)f_e=g_e=C_e(n_xa_y+a_xn_y)` and `0.5 dS2_edge/dt+nu K2_edge=R_edge`.  The exact Waleffe normalization gives `|C_e|^2/kappa_e<=2`; after child-helicity sum and the unordered `1/2` quotient, `G_(-1,edge):=int kappa_e^-1|C_Fg_e|^2dLambda_edge<=4C_F^2E||N||_2^2`, where the actual modal nonlinear source is `N=-P(u.grad u)=P(u x omega)`.  Hence `0.5 dS2_edge/dt+(nu/2)K2_edge<=(2C_F^2E/nu)||P(u x omega)||_2^2`.  Combined with the first-contact action theorem, a finite singular endpoint has `A_down=infinity` or infinite Leray-Lamb `L_t^2L_x^2` action.  The latter is not a new mechanism: `0.5 d||grad u||_2^2/dt+nu||Delta u||_2^2=<N,-Delta u>` shows finite Lamb action would keep `H^1` bounded.  The remaining frontier is rigidity inside genuine radial turnover/Lamb forcing, not a retuning-event budget.  Hodge/entropy remains downstream.
