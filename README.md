...
**DISTILLED EXACT NULL/HEAT/CANCELLATION STABILITY:** keep two layers distinct.  The same-helicity Leray **vector** source on `RP^2` carries Beltrami/shear/projective geometry and is never Hahn-split after helicity coarse-graining.  Canonical work instead uses the helicity-resolved edge source `f_e` before Hahn.  Its square law `S2_edge=int|C_F f_e|^2dLambda_edge` satisfies `S2_edge<=2C_F^2E||grad u||_2^2` and `int_0^T S2_edge dt<=C_F^2E_*^2/nu`.  For fixed child helicity `(q,s)` and heat radius `R`, the edge-fiber base mass is exactly `pi R`; if `P,N` are the positive/negative parts of the **canonical signed edge work** on that same fiber, Hilbert variance gives `PN/(pi R|a_(q,s)|^2)<=L2_edge` (and `P=N` when the fiber resultant vanishes).  Persistent unforced hiding therefore pays edge-source-square without re-Hahn.  **OPEN:** rule out finite-time accumulation of these parabolically summable edge-source/parent-forcing payments.  Hodge/entropy remains downstream.
