...
**DISTILLED EXACT PRESSURE-REDISTRIBUTION / MODAL-STRAIN LAW:** with `g=-Delta p=|S|^2-|omega|^2/2` and `H0=Hess p+(g/3)I`, each Fourier mode obeys `S_hat:H0_hat=0`, `|H0_hat|^2=(2/3)|g_hat|^2`, `|Hess(p)_hat|^2=|g_hat|^2`; hence `||H0||_2^2=(2/3)||g||_2^2` and `<S,H0>=0`, even after any common Fourier-mode restriction.  More decisively, `partial_t S=sym grad N+nu Delta S`, `N=-P(u.grad u)`, so modal strain/enstrophy production is exactly the `|k|^2`-weighted canonical kinetic work already encoded by `Z_tri`; pressure creates no new `H^1` source road.  In the physical vorticity frame, `S` and `H0` each split into exact `1+2+2` pieces `(a,b,D)` and `(tau,c,E)`, with pressure acting respectively on longitudinal stretching, direction turning and transverse strain, while `int[(3/2)a tau+2b.c+D:E]=0`.  The periodic exact field `psi=sin x+sin y-sin(x+y)` has `u=grad u=S=omega=g=0` at the origin but `Hess p(0)=[[0,-1/5,0],[-1/5,0,0],[0,0,0]]`, so no pointwise local first-jet closure or finite local pressure bound is admissible.
**EXACT EDGE-SOURCE/LAMB DIAGNOSTIC:** for the derived pre-Hahn edge source `f_e`, modal NS gives its exact damped product equation and `G_(-1,edge)<=4C_F^2E||N||_2^2`, `N=-P(u.grad u)=P(u x omega)`.  The resulting `S2_edge,K2_edge` are exact **source-level quadratic diagnostics**, not kinetic-energy/work/dissipation currencies or causal owners.  At first contact, the interval `J` and split `K<>Lambda M` are proof readers; the Cauchy lower bound on `K2_edge` is an **EXACT analytic estimate**, not a no-estimate distilled law.  Rotational NS itself gives `u x omega=N+grad B`, `B=p+|u|^2/2`, `<u,N>=0`, and `<N,-Delta u>=int omega.S omega`.  The instantaneous algebraic involution `u->-u` fixes `N` but reverses the stretching functional, so Lamb action cannot be renamed positive stretching.  Finally, finite `int_0^(T*)||N||_2^2dt` would bound `||grad u||_2` by the gradient NS identity; energy plus `||u||_Hdot^(1/2)^2<=||u||_2||grad u||_2` would then bound the Kenig--Koch endpoint norm.  Hence every finite singular endpoint already requires `int_0^(T*)||N||_2^2dt=infinity`; therefore `A_down=infinity OR Lamb=infinity` is not a new mechanism dichotomy.  The static radial/helicity/cancellation typing is closed by the signed-curl path law and Betchov bridge; pressure is now also quotiented from modal `H^1` production and classified as exact geometric redistribution.  The remaining nonredundant target must therefore be genuinely nonlocal same-field space-time structure linking late UV first contacts to the pressure/strain redistribution geometry, not a pointwise pressure bound, pressure currency or another Hodge/entropy reader.
