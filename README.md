...
**DISTILLED EXACT EDGE-SOURCE ACTION:** keep vector source geometry separate from canonical helical work.  For edge source `f_e=C_ea_xa_y`, modal NS gives `(partial_t+nu kappa_e)f_e=g_e=C_e(n_xa_y+a_xn_y)`, so `0.5 dS2_edge/dt+nu K2_edge=R_edge`, `K2_edge=int kappa_e|C_Ff_e|^2dLambda_edge`; `R_edge` is actual parent nonlinear retuning.  At a first hard-shell contact `ME_M=mu`, the existing natural window `J`, `|J|<=cM^-2`, has `M Phi_in(J)>=q_nat mu` and `ME_M<=mu` throughout the past.  For any `Lambda>2`, at least one exact action occurs: deep parents give `A_down(J)>=(Lambda-2)q_nat mu/2`, or comparable parents give `int_J K2_edge dt>=q_nat^2 mu/(64pi c Lambda^2)`.  Sparse UV scales make the comparable heat bands disjoint; hence for smooth data a finite singular endpoint forces `A_down[0,T*)=infinity` or `int_0^(T*) (R_edge)_+dt=infinity`.  This is a necessary endpoint route, not a contradiction or event budget.  Hodge/entropy remains downstream.
