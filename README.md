...
**DISTILLED EXACT NULL/HEAT/COAREA STABILITY:** intrinsic Beltrami/shear defects control surviving same-helicity source, while viscosity sorts persistent unforced cancellation fiberwise by `kappa`.  The unordered orbit-source square `S2=int dz int|C_F S_pair|^2d lambda_z^unord` obeys `S2<=2C_F^2E||grad u||_2^2`, hence `int_0^T S2dt<=C_F^2E_*^2/nu` on every compact smooth interval.  Equal-heat diamonds have an exact physical coarea: after both parent quotients and `1/2` orbit-pair symmetry, `dXi=(1/8)dq dp dp' delta(kappa-kappa')=dp dp' dH^2(y)/(16|p-p'|)`, `y perp(p-p')`.  **OPEN:** prove the resulting scale-local plane/Radon multiplicity bridge from projective incidence to actual cross-source-square, or route the only alternative—actual parent nonlinear retuning.  `S2` is not an event budget; Hodge/entropy remains downstream.
