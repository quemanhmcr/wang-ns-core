...
### A01. The extraction machine now has a common front end and two microscopes.
**TEAM DEDUCTION.** The literature supports the fork
\[
\boxed{
T_*<\infty
\to
\text{finite singular core}
\to
\text{Galilean critical activity}
\to
\begin{cases}
\text{A. singularity-preserving tangent},\\
\text{B. bounded-ancient tangent}.
\end{cases}}
\]
Branch A is available only when a scale-invariant local energy/pressure package gives compactness plus persistence of singularities.
Branch B uses record-amplitude compactness to obtain a bounded mild ancient solution and therefore needs a separate Galilean nondegeneracy mark.
### A02. The two microscopes buy different assets and should not be forced into one theorem.
**TEAM RULE.** Branch A preserves the marked singularity but generally owns more scale control. Branch B owns global-past \(L^\infty\) boundedness but may lose the singular mark into a constant mode.
**TEAM USE.** Do not require one terminal object to be simultaneously optimal for singularity persistence and ancient Liouville unless a theorem proves both.
### A02b. The remaining hard arrows are now branch-specific.
**TEAM STATUS.** For Branch B, the hard extraction seam is
\[
\boxed{\mathrm{E2}:\text{KNSS record-time compactness}+\text{nonzero Galilean mark on the same sequence}.}
\]
For the NEO layer, the main seam is
\[
\boxed{\mathrm{R1}:\text{point/contact algebra}\to\text{thick exact terminal geometry}.}
\]
...
### B08. Mildness acts as an analytic gauge fixing in uniformly-local whole-space theory.
**EXTERNAL EXACT / SCOPE.** Bradshaw--Tsai relate distributional local pressure expansion and mild formulation in uniformly-local classes; this helps separate genuine mild dynamics from parasitic pressure modes.
**TEAM USE.** For terminal work, `bounded mild ancient` is not cosmetic. It removes a large weak pressure/gauge ambiguity before NEO quotients the remaining constant Galilean mode.
Paper: Bradshaw--Tsai, *On the local pressure expansion for the Navier-Stokes equations*.
https://arxiv.org/abs/2001.11526
### B09. KNSS bounded-ancient compactness is a record-time construction.
**EXTERNAL EXACT / PROOF FEATURE.** KNSS choose running-maximal velocity times and rescale by the record amplitude; this produces uniform bounds on every fixed backward cylinder and hence a bounded ancient limit.
**TEAM USE.** E2 is not `find compactness from scratch`. The real problem is to find a Galilean-invariant mark that is nonzero on the same record-time microscope.
https://arxiv.org/abs/0709.3599
### B10. Type-I packages can remove the constant mode without a new E2 theorem.
**EXTERNAL EXACT / SCOPE.** In Type-I ancient-profile frameworks, the admissible scale-invariant decay/energy package excludes nonzero constants from the finite package.
**TEAM USE.** Galilean nondegeneracy is primarily an unrestricted/Type-II-like extraction seam, not necessarily a universal seam in every scale-controlled branch.
Albritton--Barker: https://arxiv.org/abs/1811.00502
---
...
\]
### D07. The most useful Galilean activity is the tilted-cylinder \(L^3\) quantity.
**TEAM DEDUCTION / NEAR-KNOWN.** Choose \(q=3\) in the pressure-free one-scale theorem and define
\[
\boxed{
\mathcal G_3(u;z_0,r)^3
:=
\inf_{c\in\mathbb R^3}
r^{-2}\iint_{Q_r^c(z_0)}|u-c|^3.
}
\]
Under the exact constant Galilean boost plus parabolic dilation, the tilted cylinder becomes \(Q_1\) and the displayed quantity becomes \(\iint_{Q_1}|v|^3\).
### D08. E1 is therefore close to a derived theorem, not a new compactness miracle.
**TEAM DEDUCTION.** After an explicit audit that suitability/local-energy inequalities are preserved under the constant Galilean pullback,
\[
\boxed{
z_0\text{ singular}
\Rightarrow
\mathcal G_3(u;z_0,r)^3\ge\varepsilon_3
\quad\forall r\ll1.
}
\]
The Wang--Wu--Zhou theorem is the external epsilon engine; the Galilean packaging above is the team splice.
WWZ: https://arxiv.org/abs/1811.09927
### D09. Local compactness itself is not the main E2 mystery once uniform scale bounds are owned.
**EXTERNAL EXACT / SCOPE.** Suitable-solution compactness gives strong local convergence (including \(L^3_{loc}\) in standard persistence arguments) under uniform local energy/pressure bounds.
**TEAM USE.** A local \(L^3\) Galilean mark will survive if it is carried by the very sequence for which those bounds and compactness hold.
### D10. Persistence of singularities provides a different contradiction in scale-controlled regimes.
**EXTERNAL EXACT / SCOPE.** In Type-I/local-energy compactness frameworks, singular points persist under the relevant strong local convergence; Barker--Prange and Albritton--Barker use this mechanism in localized blow-up arguments.
**TEAM USE.** On Branch A, NEO only needs to force the limit regular at the marked point; it need not produce a nonconstant bounded ancient Liouville object.
Barker--Prange: https://arxiv.org/abs/1906.08225
Albritton--Barker: https://arxiv.org/abs/1811.00507
### D11. The hard E2 mismatch is record time versus terminal endpoint.
**TEAM DEDUCTION.** KNSS record-amplitude rescaling is centered at \((x_k,t_k)\), while E1 is naturally marked by cylinders ending at \((x_*,T_*)\). In Type-II-like behavior,
\[
M_k^2(T_*-t_k)\to\infty
\]
can send the singular endpoint to infinite positive rescaled time, outside every compact window used to construct the bounded ancient limit.
**TEAM USE.** This explains why terminal critical activity can disappear into an almost-constant record-time tangent without contradicting E1.
### D12. E2 should now be called the Record-Time Galilean Mark Problem.
**OPEN.** Find a scale-invariant Galilean mark \(\mathfrak M\) such that on a KNSS-style record-time sequence
\[
|u_k|\le C\text{ on every compact past cylinder}
\quad\text{and}\quad
\mathfrak M(u_k;Q_1)\ge\varepsilon_*>0.
\]
Then local compactness yields a bounded mild ancient \(U\) with \([U]_{\rm Gal}\neq0\).
### D13. Do not overstate spatial-center escape as an open frontier without the standard whole-space continuation audit.
**TEAM RULE.** For Clay data, first pass to a suitable/Leray continuation and use the standard bounded-singular-set / regularity-at-spatial-infinity theory before declaring that first blow-up can escape to infinity. Keep this as an interface lemma to audit, not a new NEO seam.
...
### E04. Rough critical `mild` classes are not automatically canonical either.
**RECENT PREPRINT / GUARDRAIL.** Cheskidov--Hou 2026 exhibit nonuniqueness phenomena for mild/stationary singular solutions in rough negative-regularity Besov settings.
**TEAM USE.** The rigidity token is not the word `mild` alone; it is bounded mild ancient evolution with the terminal regularity/class required by the backward-uniqueness theorem.
https://arxiv.org/abs/2603.03666
---
...
### G09. Spatial analyticity makes `thick exact relation -> global terminal relation` a real R2 mechanism.
**EXTERNAL EXACT / SCOPE.** Positive-time Navier--Stokes solutions enjoy spatial analyticity; bounded ancient profiles may be viewed at terminal time as positive-time evolutions from any earlier slice.
**TEAM DEDUCTION.** If an analytic NEO relation vanishes on a terminal set of positive three-dimensional measure, it globalizes componentwise.
Grujić--Kukavica, spatial analyticity: https://doi.org/10.1006/jdeq.1997.3167
### G10. C0 therefore has a concrete thickening target.
**TEAM DEDUCTION.** If a curl-active bounded ancient terminal profile satisfied
\[
|\{x:\nabla\omega(x,0)=0\}|>0,
\]
analyticity would give \(\nabla\omega\equiv0\); then \(C^2V=C\omega=0\), bounded harmonicity gives \(V\) constant, and hence \(\omega=0\), contradiction.
**TEAM USE.** The C0 problem is no longer `differentiate again`; it is `promote one flat contact to a thick exact flat set`.
---
...
### I06. Backward uniqueness for bounded mild NS enlarges the terminal target beyond zero sets.
**EXTERNAL EXACT / SCOPE.** Lei--Yang--Yuan prove backward uniqueness for bounded mild three-dimensional Navier--Stokes solutions with prescribed final data; final-time symmetries such as axisymmetry can propagate backward.
**TEAM USE.** NEO may force a terminal datum into any class with a unique backward orbit; an exact zero set is only one special case.
https://arxiv.org/abs/2311.02429
### I07. Replace `zero-set class` by `backward-rigid terminal class`.
**TEAM DEDUCTION.** Write
\[
\boxed{\mathfrak R=\text{terminal data with a uniquely determined bounded-mild backward orbit or a known Liouville reduction}.}
\]
Useful species are explicit-orbit classes, genuine symmetry classes, and analytic relations that globalize to one of those classes.
### I08. Curl eigenfields form an explicit terminal class that is actually eliminated.
**NEO DEDUCTION + EXTERNAL BU.** If \(V=U(0)\) satisfies
\[
CV=\lambda V,
\]
then \(V\times CV=0\) and \(C^2V=\lambda^2V\). For \(\lambda\neq0\), the exact NS orbit through \(V\) is
\[
W(t)=e^{-\nu\lambda^2t}V.
\]
Backward uniqueness forces any bounded mild ancient \(U\) with final data \(V\) to equal this orbit on every finite past interval, but \(W\) grows exponentially as \(t\to-\infty\). Hence no nonzero bounded ancient terminal curl eigenfield with \(\lambda\neq0\) is admissible. For \(\lambda=0\), bounded harmonicity gives the Galilean-null class.
**PROGRESS.** This is a terminal normal form genuinely eliminated.
### I09. The explicit-orbit elimination has a wider NEO schema.
**CANDIDATE GENERALIZATION.** If terminal data satisfy
\[
P[X_V,C]V=0,
\qquad
C^2V=\mu V,
...
\mu>0,
\]
then the canonical orbit \(e^{-\nu\mu t}V\) grows backward and is incompatible with bounded ancientness once backward uniqueness applies.
### I10. Genuine terminal symmetries are also backward-rigid classes.
**TEAM DEDUCTION.** If a Euclidean symmetry \(g\) preserves NS and \(gV=V\), then \(gU\) and \(U\) have the same final data; backward uniqueness gives \(gU=U\) for all past times.
**TEAM USE.** Translation invariance, axisymmetry, or full periodicity are meaningful terminal normal forms. Mere component vanishing is not automatically a symmetry class.
### I11. Fixed-direction terminal vorticity gives a concrete symmetry reduction.
**NEO DEDUCTION.** If \(CV=f\xi\) globally at terminal time for one fixed unit vector \(\xi\), then the square anchor and bounded harmonicity imply terminal velocity is invariant along \(\xi\). Backward uniqueness propagates the translation symmetry, reducing the ancient dynamics to a 2D/2D3C class where stronger Liouville theory is available.
### I12. Fully periodic terminal data are killable by backward uniqueness plus torus energy decay.
**TEAM DEDUCTION.** Three independent terminal periods propagate backward. On the resulting torus, subtract the spatial mean and use energy plus Poincaré backward over arbitrarily long intervals; bounded ancientness forces the fluctuation to vanish. Hence the profile is Galilean-null.
### I13. Axisymmetric periodic ancient classes have an external Liouville theorem.
**EXTERNAL EXACT / SCOPE.** Lei--Ren--Zhang prove a Liouville theorem for bounded mild ancient axisymmetric solutions periodic in the axial variable.
**TEAM USE.** A finite terminal symmetry package can be enough; general 3D bounded-ancient Liouville is not required.
https://arxiv.org/abs/1902.11229
### I14. Preferred division of labor after the backward-rigid upgrade.
**TEAM DEDUCTION.** Aim for
\[
\boxed{
\text{external extraction}
\to
\text{Local-NEO contact}
\to
\text{thick exact relation}
\to
\mathfrak R
\to
\text{external backward rigidity / Liouville}
\to
\bot.
}
\]
...
### K01. Common front end.
**TEAM TARGET.** Starting from a finite singular point \(z_*\), use the exact tilted-cylinder \(L^3\) quotient to obtain Galilean critical activity at every sufficiently small scale.
### K02. Branch A -- singularity-preserving compactness.
**SCOPE.** If one owns uniform scale-invariant local energy/pressure bounds sufficient for strong local compactness and persistence of singularities, rescale at the marked singularity and keep the mark.
**NEO JOB.** Show the limit is regular at the marked point. That alone contradicts persistence; no ancient Liouville theorem is needed.
### K03. Branch B -- bounded-ancient record-time compactness.
**SCOPE.** Without the stronger scale package, use KNSS-style record-time amplitude rescaling to obtain bounded mild ancient \(U\).
**OPEN.** Carry a nonzero Galilean mark on that same record-time sequence.
### K04. Branch-B E2 theorem target.
\[
\boxed{
|u_k|\le C\text{ on every compact past cylinder}
\quad\text{and}\quad
...
\]
for one Galilean-invariant scale mark \(\mathfrak M\). Local compactness must preserve the lower bound.
### K05. E2 compactness-defect checklist.
Account explicitly for:
1. spatial center/translation;
2. parabolic dilation;
3. Galilean frame;
4. profile splitting/tightness;
5. record-time versus singular-endpoint time separation.
### K06. Once Branch B gives \([U]_{\rm Gal}\neq0\), Local NEO enters exactly as before.
For bounded incompressible mild ancient \(U\),
\[
CU\equiv0\Rightarrow C^2U=-\Delta U=0\Rightarrow U\text{ spatially constant}.
\]
Hence \([U]_{\rm Gal}\neq0\Rightarrow CU\not\equiv0\).
...
**TEAM DEDUCTION.** Use
\[
\boxed{\mathcal N=(\mathcal A,\mathcal G,\mathcal R)}
\]
where
\[
\mathcal A=\text{finite Local-NEO algebraic/contact type},
\quad
\mathcal G=\text{one finite exact scale/set geometry},
\quad
\mathcal R=\text{backward-rigid terminal class}.
\]
The target chain is
\[
\mathcal A\to\mathcal G\to\mathcal R\to\text{unique backward orbit / Liouville}\to\bot.
\]
### L08. A scale/set renderer does not enlarge the ontology.
**TEAM RULE.** Superlevel sets, positive-measure exact-relation sets, tilted cylinders, coherence, recurrence, or symmetry conditions derived from \(U,CU,S\) are renderers/conditions, not primitives.
### L09. Current terminal kill library.
**TEAM LEDGER.** Already killable or strongly reducible classes include:
- \(CV=0\): bounded harmonic/Galilean-null;
- \(CV=\lambda V\), \(\lambda\neq0\): explicit heat orbit + backward uniqueness;
- \(P[X_V,C]V=0\) and \(C^2V=\mu V\), \(\mu>0\): candidate explicit-orbit schema;
- global fixed-direction terminal vorticity: translation symmetry reduction;
- three independent terminal periods: torus energy/Poincaré rigidity;
- axisymmetric + axial-periodic terminal class: external ancient Liouville;
- positive-measure terminal set where \(\nabla\omega=0\): analyticity forces the curl-null class.
### L10. C0 next theorem target is a Thickening Lemma.
**OPEN.** From
\[
|\omega|\le1,
\quad |\omega(0,0)|=1,
\quad \omega\cdot S\omega(0,0)=0,
\]
force one exact relation to hold on a terminal set large enough for analyticity/unique-continuation machinery: positive measure, open patch, or another verified uniqueness geometry.
**TEAM RULE.** If the calculation only yields the next point jet, stop.
### L11. C1 next theorem target is a finite symmetry/orbit class, not a Riccati tower.
After the split \(\lambda_2\le0\vee\lambda_2>0\), ask whether extremal curl contact forces alignment, recurrence, translation symmetry, or an explicit orbit class on a thick terminal set.
...
### M13.
Do not force singularity persistence and bounded-ancient compactness onto the same microscope unless the required scale package is proved.
### M14.
Do not equate a terminal component constraint with a genuine invariant symmetry class. Backward uniqueness propagates symmetries of NS, not arbitrary algebraic subsets.
### M15.
Do not call `mild` alone a uniqueness token in rough critical spaces; retain the bounded terminal class and exact backward-uniqueness hypotheses.
### M16.
A new theorem-sized terminal class belongs in the kill library only if its backward orbit or Liouville reduction is explicit enough to produce contradiction.
---
## N. Team priority ranking
### N01. Priority 1 -- close E1 cleanly.
Write the exact proof that Wang--Wu--Zhou plus constant Galilean pullback gives the tilted-cylinder \(L^3\) nonvanishing lemma at a finite singular point, including suitability/local-energy transformation.
### N02. Priority 2 -- solve Branch-B E2 as the Record-Time Galilean Mark Problem.
Search for a mark that is nonzero on the same KNSS record sequence that gives global-past \(L^\infty\) compactness.
### N03. Priority 3 -- exploit Branch A whenever a scale package is available.
Use persistence of singularities and ask NEO only to regularize the marked tangent point.
### N04. Priority 4 -- prove the C0 Thickening Lemma.
Promote the flat contact from one point to a thick exact relation; use analyticity rather than higher jets.
### N05. Priority 5 -- enlarge the backward-rigid terminal kill library.
Classify explicit genetic orbits and genuine terminal symmetries before building higher descendants.
### N06. Priority 6 -- C1 finite matrix/symmetry classification.
Use \(\lambda_2\le0\vee\lambda_2>0\), alignment and exact recurrence/symmetry tests before Riccati prolongation.
### N07. Priority 7 -- global spectral upgrade only on licence.
Activate \(H,\Lambda,\Pi_\pm\) globally only after extraction supplies the required domain/integrability/tightness.
---
## O. Current theorem targets
### O01. E1 theorem -- tilted Galilean \(L^3\) nonvanishing.
**NEAR-KNOWN / PROOF AUDIT NEEDED.** For a finite singular point \(z_*\), prove
\[
\inf_{c\in\mathbb R^3}r^{-2}\iint_{Q_r^c(z_*)}|u-c|^3\ge\varepsilon_3
\quad\forall r\ll1.
\]
### O02. Branch-B extraction theorem -- Record-Time Galilean Mark.
**OPEN.** On a KNSS record-amplitude sequence, preserve one nonzero Galilean-invariant local mark while retaining uniform bounded-ancient compactness.
### O03. C0 theorem -- Thick Exact Geometry.
**OPEN.** Turn terminal flat contact into a positive-measure/open/uniqueness-set exact relation that analyticity can globalize.
### O04. Backward-rigid terminal classifier.
**ACTIVE.** Expand the finite library of terminal data classes whose unique backward orbit is explicit, symmetric, periodic, lower-dimensional, or covered by a known Liouville theorem.
---
## P. Team bottom line
The research programme is no longer a single pipeline that asks one ancient profile to carry every property. The literature supports a forked machine:
\[
\boxed{
\text{singularity}
\to
...
\to
\begin{cases}
\text{marked singular tangent}\to\text{NEO makes the mark regular}\to\bot,\\
\text{bounded ancient tangent}+\text{record-time Galilean mark}\to\text{Local NEO}.
\end{cases}}
\]
On the bounded-ancient branch, Local NEO should classify the terminal slice, not surveil the whole orbit:
\[
\boxed{
\mathcal A\to\mathcal G\to\mathcal R\to\text{backward rigidity}\to\bot.
}
\]
Here \(\mathcal A\) is a finite contact/algebraic type, \(\mathcal G\) a thick exact set geometry, and \(\mathcal R\) a backward-rigid terminal class.
The programme has now eliminated at least one genuine nontrivial terminal form: a nonzero terminal curl eigenfield is incompatible with bounded ancientness once backward uniqueness is invoked.
The two main unsolved arrows are therefore sharply separated:
\[
\boxed{\mathrm{E2}:\text{record-time bounded compactness}+\text{nonzero Galilean mark on the same sequence}}
\]
and
\[
\boxed{\mathrm{R1}:\text{single terminal contact}\to\text{thick exact terminal geometry}.}
\]
Everything that only generates another jet, another costume, or another sufficient criterion without moving E2/R1 should be treated as secondary.
