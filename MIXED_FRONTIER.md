...
1. **Good UV births.**  Use the fixed `M Phi_(in,G)` quantum plus signed-good donor geometry/common-slice registration to decide whether infinitely high fresh good births force one native parabolic ancestry to `t=0` without token matching.
2. **Bad UV births.**  Treat `M Phi_(in,B)` as real inefficient/nonforward work; find its true NS obstruction to repeated high-scale rebirth without calling transfer deficit a dissipated-energy budget.
## 13. Frontier in one sentence
> **A finite singularity now forces fresh hard-shell births at `M_n->infinity`, each carrying a fixed own-scale quantum of actual canonical inflow.  The sole remaining problem is to exclude infinite UV rebirth through the exact good/bad work alternatives—without reviving genealogy or a fake loss budget.**
