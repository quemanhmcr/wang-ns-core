...
> **FRONTIER EXHAUSTION:** there is no remaining nonredundant traffic/material/pressure/owner bridge in this architecture.  Fourier traffic, signed-curl `Z_tri`, Biot--Savart pair geometry and material secant laws are exact readings of the same full `u/omega`, but the relative-translation anti-theorem proves that the instantaneous traffic quotient is not a sufficient dynamical state even for `dot Q`; a mixed quartet can change the next source jet while all stocks, active cubic works and `Q` remain fixed.  Hence demanding a canonical spacetime bridge between these lossy readers is itself the wrong reduction.  What remains is the original direct regularity problem for the full pressure-free vorticity/Navier--Stokes PDE, not an unclosed ontology frontier.  Any future advance must be a new exact theorem on the full state; direction-coherence bounds, Cauchy/coarea/Hodge, Lamb, positive-part pair ledgers, clocks and genealogies remain auxiliary and cannot reopen the frontier.**
