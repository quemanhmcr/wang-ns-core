...
1. **Bad outflow closure.**  Classify `Phi_out,A` from the sidecar Kirchhoff law under the same canonical donor kernel; bounded-scale recycling should remain Gronwall, while UV outflow must re-enter hard-shell birth geometry.
2. **Branching versus reuse.**  The good route now supplies a second hard parent shell.  Determine whether repeated reuse of the same parent reservoir can feed arbitrarily many higher births without forcing bad-sidecar outflow/viscosity; do not count parent labels as independent energy.
## 13. Frontier in one sentence
> **Every singular UV birth emits a fixed bad boundary-flow sidecar; Kirchhoff leaves stock, viscosity or actual outflow, while every good first contact separately requires an earlier/lower critical interaction state or initial stock.  The remaining seam is the genuine bad outflow/recycling itself, not companion-parent genealogy.**
