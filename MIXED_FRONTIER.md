...
`PN=[H/(2S)]V+N_perp`,
`N_perp perp U,V`.
Thus the exact first-moment radial sorter is forced.  The stronger all-radial family then resolves the old remainder further: `dw_U=h_Ude_U` reconstructs all higher radial work shape as `N_shape=(1/2)h_shape(Lambda)U`, while `N_fib` does zero work on every radial control volume and satisfies exactly `N_fib=(U_t)_fib` because viscosity has no radial-fiber component.  Viscosity still acts with opposite sign on the first radial-shape direction.  A nondegenerate neutral tail has shallower-donor / deeper-recipient same-helicity flux reversal.

The folded radial family also gives `partial_t K_rad+2nu L_rad=Psi_rad`, `Psi_rad=int_R^infinity J`.  A fixed level `K_rad(R_kappa,t)=kappa` has unbounded endpoint `limsup R_kappa` but finite genuine viscous `int R_kappa dt`; at every outward state `R_kappa'>0`, the actual growth profile `g_tail=J-2nu D_(2,>R)` is positive somewhere above the same cut.  Every `g_tail` superlevel is a stationary-stock hardening control volume.  Either one lies wholly above `R_kappa`, or the no-cell alternative forces a monotone UV state-growth corridor with `de_t>=0` on its upper side.
