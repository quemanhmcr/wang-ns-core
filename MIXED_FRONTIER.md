...
For each heterochiral closed triad let `m` be the unique minority-helicity root.  The two exact invariants `sum T_i=0`, `sum s_i rho_i T_i=0` imply `sum rho_i T_i=2rho_mT_m`; homochiral triads contribute zero.  Hence `C_(1/2)(t)+2nu D_(3/2)=C_(1/2)(0)+2W_min^signed`.  Positive minority work has same-helicity interaction parents and is uniformly non-good: nonforward gives `r_e=0`; forward edges outside the certified stability rectangle have `J/J_*<=99/100`, while inside `u<=2/25` gives `P_same/P_opp<3/25`.  Thus every positive creator of critical stock is canonical `B` with `r_e<=99/100`, and finite-time critical blow-up forces unbounded radius-weighted positive minority-`B` work.  The `1/100` gap is native edge geometry, **not** dissipated energy or an event-count budget.
...
> **Minority-`B` critical production cannot hide inside an exact Fourier diamond: every active off-diagonal diamond has at least one mandatory extra full-NS pair source.  The sole continuum seam is quantitative near-silence/cancellation of that enlarged physical source family, not an exact rigid exception or invented entropy.**
