...
### Canonical same-output diamond cover — DISTILLED EXACT / EXTERNAL EXACT RIGIDITY
At a physical time `t0`, an absent mode `q=n+m` with a unique unordered active pair is born immediately when its exact Leray pair-source is nonzero; viscosity is zero on the absent child.  More generally, for fixed child helicity let the canonical minority-parent source measure be `dF_min=e^(i theta)d sigma`, total variation `S`, resultant `F_min`, and let `F_rest` be all other physical same-output pair sources.  Exact polar algebra gives `S^2-|F_min|^2=int int[1-cos(theta-theta')]d sigma d sigma'`.  Each nonzero off-diagonal atom is a real Fourier diamond `p+(q-p)=p'+(q-p')`.  Therefore at least one of `|F_total|>=S/4`, `|F_rest|>=S/4`, or diamond variance `>=3S^2/4` holds.  This is source/state geometry, never a causal Hahn law.  Kishimoto--Yoneda, arXiv:2110.08039, Theorem 1.4, classifies the zero-tolerance finite-mode Euler invariant endpoint as stationary 2D-like/Beltrami families.  The remaining bridge is to convert persistent physical diamond misalignment/cross-family cancellation into native NS deformation/stock/work without replacing it by cell entropy.
## 13. Frontier in one sentence
> **Critical-stock growth is localized to minority-helicity `B` work, and same-output hiding is now an exact physical diamond alternative: leak into net source, meet comparable real cross-family source, or carry quantitative pair-phase misalignment.  The remaining seam is to bind that diamond misalignment to another native NS state/work law, not to invent entropy.**
