...
This file starts where the physical core stops.  Its job is to prevent the programme from solving a recurrence that the observer accidentally created, while isolating the smallest genuine Navier–Stokes problem still open.

## 1. What has already been removed from recursion

The following may remain useful observables or provenance, but they no longer create independent event depth:

- a second Hahn split after coarse-graining;
- theorem/checkpoint count and same-carrier re-hardening;
- hard interaction cells used as persistent wallets;
- high→high circulation renamed tail supply;
- dyadic shell count used as physical progress;
- inherited stock renamed generation;
- material OO/ON/NN rereading or selected-family switching renamed source;
- smooth skew `K_phys` relink renamed generation;
- raw `material_relink` / `new_coherent_ancestry` names admitted without native evidence;
- role/probe changes admitted by label alone;
- full-natural own-scale service counted as a second event after the corridor that produced it;
- shell/service rereading counted as a fourth causal charge;
...
A useful cleanup is to stop putting all useful quantities in one “owner” enum.

### A. Kinetic-energy causality

Persistent physical modal stock, actual signed nonlinear work/boundary flux, viscous loss, and the initial surface `t=0` are the native objects in the exact energy continuity law.  After one Hahn, `dW+` is the canonical positive nonlinear recipient-work law.
...
Smooth roles, probes, coherent cells, material old/new labels, selected families, checkpoints, entropy coordinates and donor sidecars describe or disintegrate an already-existing law.  They require a separate theorem before they can be promoted to physical event semantics.

Many historical difficulties came from mixing these three layers.

## 3. Role/probe transition quotient — DISTILLED EXACT DEDUCTION

The current canonical carrier architecture admits no independent `role_change`, `probe_change` or `role_probe_change` currency.

A role transition is one of:

1. **common physical transport/gauge:** `dot A+[G,A]=0`; its observer contribution cancels exactly;
2. **smooth physical relink:** after gauge quotient, `K_phys` gives antisymmetric same-event boundary flux, zero generation depth;
3. **coefficient obstruction:** a locator that must reenter the actual `Q^2` energy/work gate;
4. **hard-to-smooth registration:** performed after an actual hard event or critical shell already exists;
5. **arbitrary unbound replacement:** invalidates the previous certificate and fails closed; it does not become an owner.

A probe is only the readout `z=<Qu,psi>`.  Holding `Qu` fixed while changing `psi` can change `z` from full amplitude to zero without changing carrier energy, nonlinear work, dissipation or shell balance.  Therefore probe identity or coefficient magnitude cannot own a physical event.

This quotient removes an observer-facing escape route; it does not close Mixed recurrence.

## 4. Shell/service witness–charge quotient — DISTILLED EXACT DEDUCTION

A critical shell and a positive coherent/increment service law are real physical observables.  They are not representation artifacts.  But current master-facing routes use them as **state/witness consequences** of an already-typed physical law, not as independent charges.

Canonical pattern:

```text
already-typed source/work/dissipation law
        -> positive service or hard-shell consequence
        -> physical shell/tail state
        -> actual first stop, t=0, or no-hit corridor
```

Objective SGS/source work may force coherent service; fresh service may force a hard shell; pressure-pair or resolved-dissipation structure may force a shell directly.  These consequence maps may change units and may expose a state at a later physical time, but they do not clone the upstream charge merely because the same physics became readable in another observable.

If a critical shell completes a full no-hit natural corridor, the own-scale service lower lives on that very corridor and adds zero recursion depth.  If high-frequency service reaches the physical hard tail, the exact tail identity resolves the energy supply into inherited stock or true low→high nonlinear work.

A shell/service observation with no typed upstream law fails closed.  The word “service” is not permission to invent a source.

## 5. Hard-shell supplier/owner quotient — DISTILLED EXACT DEDUCTION

Mode-set continuity sharpens the previous section.  For a fixed hard shell,

\[
E_M(t)+D_M+\Phi_{out,M}=E_M(s)+\Phi_{in,M}.
\]

If `M E_M(t)>=mu_0`, then for every `0<theta<1`,

\[
\boxed{M E_M(s)\ge\theta\mu_0\quad\text{or}\quad M\Phi_{in,M}\ge(1-\theta)\mu_0.}
\]

Therefore distinguish two questions:

1. **Which true PDE theorem forced or located this shell and its geometry?**  Possible answers include resolved dissipation, pressure-pair structure, fresh SGS service, high-tail supply.
2. **What supplied the kinetic energy currently stored in the shell?**  Exact answer: earlier modal stock or actual nonlinear boundary inflow.

A theorem supplier may keep essential scale/geometry provenance, but it cannot remain a third owner of shell kinetic energy merely because it exposed the shell.

This is the cleanest current example of “physical object is not automatically physical owner.”

## 6. Source/SGS audit: what is already clear

The full Leray velocity equation has quadratic nonlinearity plus viscosity.  Smooth global SGS transfer is exactly the graded spectral nonlinear flux, so **SGS transfer is not a second transfer currency beside the nonlinear work law**.  Changing an admissible resolved cutoff repartitions the same `-Q B(u,u)` forcing rather than changing the underlying NS interaction.
...
If a positive law is used, state where its Hahn split occurred.  Downstream decompositions must be restrictions/pushforwards of that law unless a genuinely new signed physical law has been derived.

If a recurrence step is claimed, identify what changes physically between the two event times.  A state rereading, capacity threshold, shell label, material age or theorem checkpoint is not enough.

For any alleged finite budget, exhibit the actual globally bounded resource or exact scale/time geometry.  `O(1)` per critical event does not imply finitely many events.

Finally ask the Tao/averaged-NS stress test:
...
## 12. Traffic/material quotient exhaustion
### Material law + relative-translation anti-correspondence — DISTILLED EXACT DEDUCTION
The material-pair law remains exact: for material `x,y`, `K=[(delta omega).h][(omega_x cross omega_y).h]/|h|^5` obeys the secant-gradient equations, common incompressible affine deformation cancels from chirality generation, and only segment gradient-curvature or viscosity can leave the chirality-null manifold.  But this geometry is **not** canonically downstream of the instantaneous traffic quotient.  Exact finite-Fourier anti-correspondence: use signed supports of `A={(-1,-1,-1),(0,1,1),(1,0,0)}` and `B={(-7,1,-3),(3,0,2),(4,-1,1)}` with rational divergence-free carriers `(k_2,-k_1,0)` and phase `i` on each third root.  There is no mixed active closed triad.  Translating only `B` by `(pi,0,0)` preserves every modal stock and every active cubic work, hence normalized `Q=-980`; nevertheless the mixed quartet `(1,0,0)+(0,-1,-1)+(3,0,2)+(-4,1,-1)=0` makes the true NS first jet phase-sensitive, and exact rational algebra gives `dot Q_delta-dot Q_0=87152/55` for every `nu>=0`.  Therefore even full instantaneous stock+canonical-work+`Q` does not determine `dot Q`, spatial pair density or segment curvature: the cubic traffic reader has discarded physical relative-phase information still present in the full state.  The earlier opposite-`Z_tri` witness separately forbids any atomwise positive-stretching map.
...
> **FRONTIER EXHAUSTION:** there is no remaining nonredundant traffic/material/pressure/owner bridge.  The exact unresolved block is now narrower than “direct regularity” in the abstract: finite singularity has already been pushed to `Y`, then to the universal bad sidecar and `S vee V vee O`; positive critical regeneration is the existing minority-`B` branch.  The only missing theorem is a full-state proof that this `B/S/V/O` physical grammar cannot recycle critically to a finite endpoint.  Any valid advance must act on the genuine `u/omega/N/G` dynamics and exact convolution/strain geometry; clocks, genealogies, new Hahn laws, transfer wallets and traffic-to-material bridges remain forbidden.**

## 14. Direct full-state `B/S/V/O` frontier — CURRENT OPEN BLOCK

This section replaces the obsolete phrase “the remaining problem is just direct regularity” by the exact road that must still be closed.  The endpoint chain is already
`T_*<infinity => X vee Y`, `X=>bot`, `Y=>S vee V vee O`.
`B` below is not a fourth fate: it is the already-canonical minority-helicity positive-critical work branch through which critical regeneration can occur inside the surviving roads.

### Critical action puts `S` and `V` on one exact full-state law — EXACT

With `N=-P(u.grad u)=u_t+nu Lambda^2u`,
`||N||_Hdot^(-1/2)^2=||u_t||_Hdot^(-1/2)^2+nu^2||u||_Hdot^(3/2)^2+nu d||u||_Hdot^(1/2)^2/dt`.
The fixed-set version after `P_A` is exact.  Sidecar `S` is the final `Hdot^(1/2)` stock term and `V` is the positive viscous `Hdot^(3/2)` term.  The RHS is a full-state quartic acceleration action, not a finite budget.

### All three roads re-enter through one genuine field — EXACT

`G:=curl N=omega_t-nu Delta omega=S omega-(u.grad)omega`.
It satisfies
`G_t-nu Delta G=[G,u]+[omega,N]-2nu sum_j[partial_j omega,partial_j u]`.
Thus stock persistence, re-creation after viscous loss and true nonlinear outflow all require the same nonlinear vorticity acceleration.  Pressure is absent from this closed evolution and remains only the Hodge completion of the velocity/strain description.

### Pressure is a reflection, not a supplier — EXACT

For `M=S^2+Omega^2`, `R=M+Hess p=-(D_tS-nu Delta S)`, Piola gives per Fourier output
`Rhat=Mhat-2<Mhat,e tensor e>e tensor e`, `e=q/|q|`.
Hence `|Rhat|=|Mhat|`.  This is structural rigidity but not an endpoint budget.

### Dangerous `O` is an opposite-high `B` arch — EXACT ALGEBRAIC CONSEQUENCE

A true far-UV low-donor triad with opposite-helicity highs has the low root at signed middle, `Z_tri>0`, both highs recipients, nonnegative radial flux across every cut and positive critical minority-`B` production.  A same-helicity high pair is a radial loop with a farther high donor and critical production `<=0`.  Consequently a true `O` interaction whose recipient is radially outermost in its triad must be the opposite-high positive-critical `B` arch.

### Silent-gate law and stock/phase split — EXACT

For a selected triad let `mathfrak Z=Z+iJ` be the complex interaction amplitude and `B_tau=c_alpha E_beta E_gamma+c_beta E_gamma E_alpha+c_gamma E_alpha E_beta`.  Then
`dot Z+nu Sigma Z=(|Lambda_tau|^2/2)B_tau+M_tau`,
where `M_tau` is exact mixed-channel phase interference.  At an active silent gate, `dot Z=J Omega`, with `Omega=sum_i Im(conj(a_i)n_i)/E_i`; direct viscosity does not rotate modal phase.
Forward selected work self-brakes the neutral stock geometry, while differential viscosity has the opposite sign at the dangerous opposite-high neutral surface.  The selected viscous commutator reads the same cross-effect; it must not be counted as a second mechanism.

### Radial compensation cannot become harmless on only three roots — EXACT ALGEBRAIC CONSEQUENCE

If a work vector is supported on three distinct signed roots and satisfies nonlinear energy and helicity conservation, it is a scalar multiple of the canonical cyclic vector `c=(c_alpha,c_beta,c_gamma)`.  The same holds for every work time jet while its support remains there.  Hence a radial counterwork that stays on the selected dangerous three roots either vanishes or reproduces the same canonical direction; a genuinely different radial compensation requires additional signed-root work support.  Energy/helicity do not constrain tangential phase countersteering, so the latter must be read from full `G/F_N`.

### Full-polarization outward leak and corrected local terminal class — EXACT ALGEBRAIC CONSEQUENCE

The helical mirror atom is only the first reader.  Summing both helicities at a geometric pair gives the exact full-pair Pythagoras
`|N_K^(p,q)|^2=|p x q|^2[|BC+DA|^2+((|q|^2-|p|^2)^2/|K|^2)|BD|^2]` in the pair-plane frame.  Unequal-radius full-pair silence is exactly common-normal shear; equal-radius pairs have the wider null manifold `BC+DA=0`, so equiradial mirror cancellation does not determine low critical-work sign.  For a nondegenerate closed geometric triad with all three Fourier coefficients active, any nonzero internal full-pair acceleration forces, by reality, a nonzero companion pair at `k_i-k_j` with radius strictly larger than every triad root.  There is no uniform multiplicative jump.  A real one-triad finite-Fourier state can close only by becoming nonlinear-silent: non-equilateral closure is planar shear, while equilateral closure includes the monochromatic `2D3C` family and is not restricted to Beltrami.

### Heat-fiber cancellation and parent repair — EXACT / EXACT ANALYTIC / OPEN

At a fixed child, the physical pair heat rate is `kappa=|p|^2+|q|^2`.  Along the frozen heat orbit the child source is the vector Laplace transform `int exp(-nu kappa tau)dmu_K(kappa)`; under finite-variation integrability, vanishing on an interval forces every heat-fiber resultant to vanish by Laplace uniqueness.  Thus passive cancellation across unequal heat rates is impossible.  For an actual helical pair atom `F_e=C_e a_p a_q`, `dot F_e=(r_p+r_q)F_e`, `r_k=dot a_k/a_k`.  A minimal persistent cancellation circuit therefore synchronizes all pair sums `r_p+r_q`.  Unequal-heat synchronization requires actual nonlinear radial parent work; only same-heat cancellation can remain purely phase-driven.  With `eta_k=n_k/a_k` and `h_k=(F_N)_k`, `dot eta_k=h_k/a_k-eta_k^2`, so non-affine repair is controlled by the genuine `F_N/G` source.

If overlapping circuits are additionally assumed to force the affine law `r_(k,s)=sigma+i v.k`, true viscous NS reduces the state to monochromatic amplitude decay plus rigid translation and all nonlinear modal works vanish; on finite-energy `R^3` the exact monochromatic state is trivial.  This conditional theorem does not prove that a general continuum cancellation network synchronizes.  The remaining hard mechanism is therefore same-heat, non-affine, dynamically `F_N/G`-repaired cancellation at the dangerous `B/O` frontier.

### Exact remaining obstruction — OPEN

The missing theorem must show, using only the full true-NS state and its exact identities, that continuum same-heat non-affine repair/cancellation cannot sustain an infinite sequence of critical `B/S/V/O` re-entry toward a finite singular endpoint.

Required final statement:
`No infinite critical full-state recycling`, equivalently `Y=>bot` under the already-proved endpoint grammar.  Once this is proved, the conditional composition in `FOURTH_DOCUMENT_FINAL_EXHAUSTION.tex` becomes the unconditional QED.  See `BSVO_FULL_STATE_FRONTIER.md` for the dedicated derivations and `SOLUTION_MAP.md` for the short proof graph.
