...
> **FRONTIER EXHAUSTION:** there is no remaining nonredundant traffic/material/pressure/owner bridge.  The exact unresolved block is now narrower than “direct regularity” in the abstract: finite singularity has already been pushed to `Y`, then to the universal bad sidecar and `S vee V vee O`; positive critical regeneration is the existing minority-`B` branch.  The canonical full-state frontier is now organized from the mother law `N=u_t+nu Lambda^2u`: its signed-curl action profile forces a kinetic-work-neutral but helicity-hardening single-helicity tail, and on that tail `PN=[H/(2S)](Lambda-mu)U+N_perp` isolates all first-moment radial hardening from the moment-orthogonal phase/polarization remainder.  The only missing theorem is to rule out repeated unbounded-scale regeneration of that forced sorter by the true `N_perp/F_N/G` dynamics.  Clocks, genealogies, new Hahn laws, transfer wallets and traffic-to-material bridges remain forbidden.**

## 14. Direct full-state `B/S/V/O` frontier — CURRENT OPEN BLOCK

The detailed canonical derivation now lives in `BSVO_FULL_STATE_FRONTIER.md`; this mixed document records only how the older cancellation frontier embeds into the newer mother-law spine.

### Mother law, not a parallel cancellation architecture — EXACT

For every fixed Fourier/helicity set `P`,

`nu W_P=||Lambda^-1PN||^2-||Lambda^-1Pu_t||^2+nu^2||Lambda Pu||^2`,

with the same `W_P` appearing in exact mode-set Kirchhoff continuity.  Pushing actual work to signed curl `x=s|k|` gives the hinge potential `Psi(a)` and quadratic action profile `A(a)=2nu Psi(a)`.  Its slope is actual cut flux and its curvature is actual modal work.  Critical production is the profile height at `a=0`; vortex stretching is its area.

### Positive criticality isolates the true hardening direction — EXACT on smooth finite-energy `R^3`

Positive critical production forces a single-helicity tail `P_(sigma,R_*)` with
`2<U,PN>=0` but `H=2<Lambda U,PN>>0`, `U=Pu`.
Set
`mu=<Lambda U,U>/||U||^2`, `V=(Lambda-mu)U`, `S=||V||^2`.
Then

`PN=[H/(2S)]V+N_perp`,
`N_perp perp U,V`.

Thus the exact first-moment radial sorter is forced; all same-heat phase/polarization/higher-shape complexity that survives the two primitive moments lies in `N_perp`.  Viscosity decomposes on the same direction with opposite sign.  A nondegenerate neutral tail has shallower-donor / deeper-recipient same-helicity flux reversal.

### The older heat/full-pair frontier survives only inside the remainder — EXACT / OPEN

The full-polarization pair Pythagoras still distinguishes unequal-radius common-normal silence from the wider equiradial phase-null manifold.  Reality still forces an outward companion pair from every fully active closed triad.  Passive cancellation across unequal heat rates is still impossible by heat-fiber Laplace uniqueness; persistent unequal-heat cancellation requires true parent radial work.  Same-heat non-affine cancellation can still use phase/polarization freedom, but this freedom is now typed as part of the moment-orthogonal `N_perp` and its true `F_N/G` evolution rather than as a second mechanism.

The fixed-mode work curvature law
`(partial_t+2nu|k|^2)T_k=2|N_k|^2+2Re(conj(u_k)(F_N)_k)`
shows exactly what `F_N` must do to preserve or rebuild donor curvature.  Under the explicit affine synchronization hypothesis, the state remains terminal regular monochromatic translation/decay; no such affine conclusion is known for the general continuum remainder.
...
The sharp unresolved subproblem is no longer merely “same-heat cancellation exists”; it is whether true moment-orthogonal `N_perp/F_N` dynamics can repeatedly regenerate the forced neutral-tail sorter `[H/(2S)](Lambda-mu)U` at unbounded UV scales while satisfying all exact full-pair, heat-fiber, pressure/strain and viscosity identities.  See `BSVO_FULL_STATE_FRONTIER.md` §§2--20 and `SOLUTION_MAP.md` for the current proof graph.
