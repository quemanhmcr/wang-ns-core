...
1. **Bad outflow closure.**  The stock and viscosity sidecar fates are now native states/sinks; classify the remaining `Phi_out,A` by the same canonical donor flow and determine whether repeated helical-annulus outflow must return to `G`, another bad boundary crossing, or a hard-shell state.
2. **Companion-parent hardness.**  On a `G`-dominant natural birth window, quantify from actual work the critical mass required in the second interaction parent; this is the branching input not supplied by a one-parent genealogy.
## 13. Frontier in one sentence
> **Every singular UV birth now emits a fixed bad boundary-flow quantum into a comparable Fourier–helicity set, and Kirchhoff leaves only stock, viscosity, or actual outflow.  The remaining interior endpoint seam is therefore concrete: classify that outflow and the second-parent mass needed for the next good birth.**
