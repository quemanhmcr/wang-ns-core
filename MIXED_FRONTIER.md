...
## 12. Pressure redistribution quotient + nonlocal closure
### Modal strain source + vorticity-frame spin law — DISTILLED EXACT DEDUCTION
The signed-curl path/Betchov theorem remains the exact H1 source spine: closed-triad work is the oriented path `alpha--beta--gamma`, radial traffic is its fold `rho=|x|`, and `C_F int Z_tri dLambda_tri=2 int omega.S omega`.  Pressure is now fully typed as redistribution rather than a competing source.  With `g=-Delta p=|S|^2-|omega|^2/2` and `H0=Hess p+(g/3)I`, every nonzero Fourier mode satisfies `S_hat:H0_hat=0`, `|H0_hat|^2=(2/3)|g_hat|^2`, `|Hess(p)_hat|^2=|g_hat|^2`; therefore `||H0||_2^2=(2/3)||g||_2^2` and `<S,H0>=0`, even after any common mode restriction.  The projected PDE gives the stronger quotient `partial_t S=sym grad N+nu Delta S`, `N=-P(u.grad u)`: modal strain-square production is exactly `|k|^2/2` times modal velocity-square production, so all H1/enstrophy source is already the canonical `|k|^2`-weighted nonlinear work and pressure opens no source road.  At `W=|omega|>0`, `xi=omega/W`, `P=I-xi xi`, decompose `S` into `(a,b,D)` and `H0` into `(tau,c,E)` by `a=xi.Sxi`, `b=PSxi`, `D=PSP+(a/2)P`, `tau=xi.H0xi`, `c=PH0xi`, `E=PH0P+(tau/2)P`.  Exact norms/pairing are `|S|^2=3a^2/2+2|b|^2+|D|^2`, `|H0|^2=3tau^2/2+2|c|^2+|E|^2`, `S:H0=3a tau/2+2b.c+D:E`, with global `int(3a tau/2+2b.c+D:E)=0`.  The full viscous NS laws are `D_tW=aW+nu xi.Deltaomega`, `D_txi=b+(nu/W)PDeltaomega`, `D_ta=-a^2/2+5|b|^2/3+|D|^2/3-W^2/6-tau+nu[2b.Deltaomega/W+xi.(Delta S)xi]`, while the covariant spin-1/2 equations are `P D_tb=-2ab-c+viscosity` and `P D_tD P=aD-3 STF_P(b tensor b)-E+viscosity`.  Thus pressure channels act only as longitudinal/turning/transverse strain geometry and their net strain-Frobenius work is zero.  Local algebra is exhausted too: the exact periodic field `psi=sin x+sin y-sin(x+y)`, `u=(psi_y,-psi_x,0)`, has `u=grad u=S=omega=g=0` at the origin but `Hess p(0)=[[0,-1/5,0],[-1/5,0,0],[0,0,0]]`; amplitude scaling makes the tidal Hessian arbitrarily large while the local first jet stays zero.  Hence no pointwise `Hess p=F(S,omega)` or finite local pressure bound can close the endpoint.
...
> **Pressure/H1-source frontier closure:** the exact NS pressure Hessian is now fully quotiented from modal enstrophy production: each Fourier strain coefficient is orthogonal to the pressure tidal tensor, while `partial_t S=sym grad N+nu Delta S` shows that H1 source is exactly the `|k|^2`-weighted canonical Leray work already classified by the signed-curl path.  Physical-space pressure remains real geometry and is completely typed by the vorticity-frame spin-0/1/2 channels `(tau,c,E)`, with exact tidal isometry and zero global strain pairing.  The three-sine anti-theorem proves that local first-jet geometry cannot determine or pointwise bound that tidal field.  Therefore the local pressure/same-field algebra frontier is closed.  The next nonredundant theorem, if one exists, must use genuinely **nonlocal space-time compatibility** between late UV first-contact dynamics and the same-field quadrupole/strain redistribution; Lamb, `J,Lambda,S2_edge,K2_edge`, Cauchy, coarea, pressure-pair entropy and the spin components remain diagnostics/readers, never owners or currencies.**
