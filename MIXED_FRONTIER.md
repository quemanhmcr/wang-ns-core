...
> **Audit correction:** low-recipient radial turnover is genuinely typed by the closed-triad invariants, but Lamb `L_t^2L_x^2` divergence is already a generic necessary signature of any finite singular endpoint, not a second mechanism road.  The exact first-contact `Lambda` split only says: deep contacts force real radial action, while comparable contacts force a lower bound on a derived edge-source damping diagnostic that is controlled by the actual Leray-Lamb source.  `J`, `Lambda`, `S2_edge` and `K2_edge` are proof/diagnostic objects, never owners or currencies.  The next theorem must extract additional true-NS geometry, not rebrand these diagnostics as physics.**
