...
## 12. Material-pair vorticity closure + first-contact anti-bridge
### Secant-gradient chirality dynamics — DISTILLED EXACT DEDUCTION
The signed-curl/Betchov theorem remains the exact `H1` source spine, and the Biot--Savart source now has an exact material-pair dynamics.  With material points `x=X(a,t), y=X(b,t)`, `h=y-x`, define `L=(omega_y-omega_x).h`, `C=(omega_x cross omega_y).h=((bar omega) cross (delta omega)).h`, `K=LC/|h|^5`; volume preservation gives `Q=(3/8pi)int int K da db`.  Put `Abar=int_0^1 grad u(x+theta h)dtheta`, `Sbar=sym Abar`, so `hdot=Abar h`, `tr Abar=0`, `rdot/r=n.Sbar n`.  The exact endpoint deviations are `E_x=-int_0^1(1-theta)(h.grad)A(x+theta h)dtheta`, `E_y=int_0^1 theta(h.grad)A(x+theta h)dtheta`.  Then `Cdot=det(E_x omega_x,omega_y,h)+det(omega_x,E_y omega_y,h)+nu[det(Deltaomega_x,omega_y,h)+det(omega_x,Deltaomega_y,h)]`, while `Ldot=2(deltaomega).Sbar h+(E_y omega_y-E_x omega_x).h+nu(delta Deltaomega).h` and `Kdot=|h|^-5(C Ldot+L Cdot)-5(n.Sbar n)K`.  Hence common incompressible affine deformation can change the longitudinal factor and metric but cancels exactly from chirality generation; leaving `C=0` requires actual velocity-gradient curvature along the material segment or viscosity, whereas leaving `L=0` may occur through common strain.  This is a true-NS zero-set classification, not a coherence estimate.  Direct first-contact coupling is ruled out exactly: on the homochiral radii `1<sqrt(2)<sqrt(3)` with cut `6/5`, choose `Z_A<0` so `1->sqrt(2)` has crossing mass `m`; dilate by `sqrt(2/3)` and choose `Z_B>0` so `2/sqrt(3)->sqrt(2)` has the same cut, child radius and mass, then rotate to the same child wavevector/helicity.  Nonzero Waleffe coupling realizes both signs.  Thus canonical positive boundary inflow cannot be pushed atomwise to positive `Q` or positive spatial chirality.  Nevertheless a finite singular endpoint forces `sup_(t_0<t<T*) int_(t_0)^t Q=+infinity` for every `t_0<T_*`, so after every late first contact the future cumulative signed material-pair source is unbounded along a subsequence, without a contact-to-pair genealogy.
...
> **First-contact/two-point frontier closure:** the missing bridge is now typed both positively and negatively.  Positively, the primitive source `Q` has exact material-pair dynamics: longitudinal increment evolves under common strain plus endpoint curvature/viscosity, while vortex-pair chirality is invariant under the common incompressible affine part and can be created only by actual gradient curvature along the segment or viscosity.  Negatively, even the same child wavevector/helicity, radial cut and positive boundary-crossing mass admit opposite `Z_tri` signs, so no atomwise UV-birth -> positive-stretching/chirality owner map exists.  Finite singularity couples the two descriptions only cumulatively through the exact `H1` stock law, forcing unbounded future signed pair source after every late time.  Any next nonredundant theorem must therefore prove an exact spacetime correlation between late radial traffic and segment-curvature/material dynamics, or exhibit an exact compatible escape; direction-coherence bounds, Cauchy/coarea/Hodge, Lamb and positive-part pair ledgers remain auxiliary readers.**
