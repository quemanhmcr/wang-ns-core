...
1. **Bad-sidecar recycling.**  Start from the universal `M W_B>=3c_nu/25` output of every UV birth and apply mode-set continuity to its recipient stock: it must remain, dissipate, or leave by actual flow; none may be renamed transfer loss.
2. **Companion-parent hardness.**  On a `G`-dominant natural birth window, quantify from the actual trilinear work how much critical mass the second interaction parent must carry; this is the true branching resource behind any further UV step.
## 13. Frontier in one sentence
> **Every sufficiently large singular UV birth now carries an unavoidable fixed bad-positive sidecar, even when its main inflow is signed-good.  The remaining question is physical recycling: can that real side energy and the required companion-parent mass support infinitely many higher-scale births without viscosity or stock/flow laws stopping the process?**
