...
For fixed child helicity, `dF_min=e^(i theta)d sigma` obeys `S^2-|F_min|^2=int int[1-cos(theta-theta')]d sigma d sigma'`, so hidden minority source gives net source, comparable other same-output source, or `>=3S^2/4` physical diamond variance.  The same parent atom has the exact spin-two companion `F_h=-Lambda e^(2ih phi)F_-h`, `Lambda=(N+M+Q)/(N+M-Q)>1`.  Every off-diagonal diamond `p+r=p'+r'=q` also forces cross pairs at `q+-d`; for unequal-scale nonparallel same-helicity parents, `||F_(n,m)||>=(1/2)|sin angle(n,m)| |M^2-N^2|/|n+m| |a_na_m|`, and zero tolerance forbids all non-original pair sources from vanishing simultaneously.  The two apparent weak faces are genuine nonlinear-null manifolds.  For a divergence-free helicity component `v_h`, define `B(v_h)=inf_K ||(curl-hK)v_h||_2` and `S(v_h)=inf_|e|=1 [int(1-(k.e)^2/|k|^2)|vhat_h(k)|^2dk]^(1/2)`.  With the native unitary factor `C_F=(2pi)^(-3/2)`, exact `N(v_h)=P(v_h x curl v_h)=P(v_h x (curl-hK)v_h)` and convolution Cauchy give `sup_q|Nhat(q)|<=C_F||v_h||_2 B(v_h)`; if `supp vhat_h subset {|k|<=Kmax}`, incompressibility gives also `sup_q|Nhat(q)|<=2C_F Kmax||v_h||_2 S(v_h)`.  `B=0` iff one curl eigensphere (Beltrami), `S=0` iff one Fourier line (shear); both are exact nonlinear-null states.  Cancellation cannot remain hidden dynamically for free either.  For one actual parent source atom `f_q(p)=C_q(p)a_p a_(q-p)`, modal NS gives `(partial_t+nu kappa_q(p))f_q(p)=C_q(p)[N_p a_(q-p)+a_p N_(q-p)]`, `kappa_q(p)=|p|^2+|q-p|^2`.  Hence on compact comparable support, if the parent-forcing right side vanishes and the net child source vanishes on an open time interval, uniqueness of the Laplace transform forces `kappa_#dF=0`: cancellation is fiberwise at equal physical heat rate.  Otherwise actual nonlinear parent source is the only retuning term.  These are source/state laws, never a new Hahn/work currency.  Kishimoto--Yoneda supplies only the external zero-tolerance finite-mode rigidity endpoint.  The remaining seam is quantitative measure-level cancellation **inside heat-rate fibers or under actual parent forcing**; Hodge/entropy is not primitive.
