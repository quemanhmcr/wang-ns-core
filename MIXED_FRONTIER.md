...
At a physical time `t0`, if an absent mode `q=n+m` has a unique unordered active pair and its exact Leray pair-source is nonzero, the Fourier NS ODE gives `d_t u_q(t0)!=0`; viscosity is zero on the absent child.  This is state birth, not `dW+`.  Kishimoto--Yoneda, arXiv:2110.08039, Theorem 1.4, classifies exact finite-mode Euler invariance as stationary 2D-like/Beltrami families.  The continuum bridge is now narrower: quantitative same-output cancellation must be proved specifically for the proliferating minority-`B` production law—either new modal stock leaks out, or comparably large physical pair sources cancel at the same output.  Cell count/entropy may only read those pairs after this positive physical cover exists.
## 13. Frontier in one sentence
> **Critical-stock growth is now localized to one true NS channel: radius-weighted positive minority-helicity `B` work with a fixed native efficiency gap.  Unique pair sources leak immediately; the sole continuum seam is quantitative same-output cancellation among many real minority-production pairs, not generic shell proliferation or an invented entropy.**
