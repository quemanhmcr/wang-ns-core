...
## 12. Pressure-free vorticity + Biot--Savart nonlocal closure
### Vorticity-only state, spatial chirality, and material law — DISTILLED EXACT DEDUCTION
The signed-curl/Betchov theorem remains the exact `H1` source spine, but the nonlocal same-field question now closes without pressure.  On smooth decaying `R^3` states, `u=curl(-Delta)^-1 omega`, `S=sym grad curl(-Delta)^-1 omega`, and NS is equivalent to the pressure-free closed equation `partial_t omega+(u.grad)omega=(omega.grad)u+nu Delta omega`; conversely this equation reconstructs the momentum law with `-Delta p=|S|^2-|omega|^2/2`.  At each Fourier output the raw convection is the orthogonal Hodge sum of Leray source and longitudinal pressure, so pressure is reconstruction/redistribution, never an extra `H1` source.  Biot--Savart gives `S(x)=(3/8pi)PV int[(h cross omega(x+h)) tensor h+h tensor(h cross omega(x+h))]/|h|^5 dh`; for `xi=omega/|omega|`, longitudinal strain sees only the pure Cartesian `l=2` angular kernel `(xi.n)(xi cross n)`.  A spatial pair has zero stretching if `xi.n=0` or the remote vorticity lies in `span{xi,n}`; otherwise its sign is the exact oriented triple-product sign.  Globally, with `h=y-x`, `Q=int omega.S omega=(3/8pi)int int[((omega_y-omega_x).h)((omega_x cross omega_y).h)]/|h|^5 dxdy`; the diagonal is genuinely softened because both factors are `O(|h|^2)`.  This equals `(C_F/2)int Z_tri` only as a global scalar identity, not as a triad-to-spatial-pair coupling.  Along the physical flow `F=grad_a X`, `det F=1` and `d(F^-1 omega(X,t))/dt=nu F^-1 Delta omega`; pressure is absent and viscosity is the sole defect of the Cauchy invariant.  Thus local pressure algebra, nonlocal pressure reconstruction, and `H1` source typing are all closed without a new owner or budget.
...
> **Vorticity-only nonlocal frontier closure:** pressure is now completely downstream of the true dynamical state.  The exact state equation is the Biot--Savart vorticity PDE; its stretching scalar has both the signed-curl Fourier representation and the symmetric two-point physical-space representation, whose nonzero atoms require longitudinal vorticity increment and genuine three-dimensional vortex-pair chirality.  The Cauchy material law further says that incompressible deformation carries the inviscid vorticity exactly and viscosity is the sole frozen-in defect.  Hence neither pressure, Lamb action, nor the positive part of the spatial chirality integrand may open a new owner/budget/Hahn road.  The next nonredundant frontier, if it exists, is sharply narrowed to an exact theorem connecting arbitrarily late UV first-contact traffic to this Biot--Savart two-point/material vorticity geometry (or proving an exact compatible escape); generic direction-coherence estimates, Cauchy bounds, coarea and entropy are not substitutes.**
