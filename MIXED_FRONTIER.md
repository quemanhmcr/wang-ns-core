...
\]
This is already a causal corridor made only of physical objects.  For a hard annulus `P_M`, after the last `theta`-entry it stays occupied, so shell viscosity gives `M D_M>=(nu theta mu/2)M^2(t-s_theta)`, `mu=M E_M(t)`.  Therefore every natural length `cM^-2` admits, by a sliding/partition proof only, some physical subinterval `J` of length `<=cM^-2` with `M Phi_in,M(J)>=q_nat mu`, `q_nat=min(1-theta,nu theta c/4)`.  Short birth corridors supply the quantum directly; long corridors supply it through viscous maintenance.  The window is not an event.  Pressure/SGS, `S`, service, material marks and relink still add no supplier term.
...
## 12. Minority-helicity production + full-convolution frontier
### Critical-stock production channel — DISTILLED EXACT DEDUCTION
For each heterochiral triad let `m` be the minority root.  `sum T_i=0` and `sum s_i rho_iT_i=0` give `sum rho_iT_i=2rho_mT_m`; homochiral triads contribute zero.  Writing `C_+-/D_+-` by helicity, the same algebra refines globally to `C_+(t)+2nu D_+=C_+(0)+W_min^signed` and `C_-(t)+2nu D_-=C_-(0)+W_min^signed`: nonlinear critical production is injected equally into both helicity sectors.  Positive minority work is canonical `B` with the existing uniform `r_e<=99/100` gap.  More precisely, if its radius is `a` and the two majority radii are `p<q`, then `T_q=(a+p)T_m/(q-p)>0`, `T_p=-(a+q)T_m/(q-p)<0`; the canonical donor flow `p->q` has radial action `(q-p)T_q=(a+p)T_m>aT_m`.  Hence for every `R>0`, the positive minority sublaw with `p>=Ra` obeys `W_(min,+)^deep(R)<=V_rho/(R+1)`.  Deep high-high->low critical creation is therefore already an existing radial-variation/viscosity route; the independent full-convolution seam may be restricted to `p<Ra`, where all three radii are comparable by `<R+1`.  The same donor row has a stronger spectral-order law.  With donor mass `D=-T_p=T_m+T_q`, recipient probabilities are `pi_a=(q-p)/(a+q)`, `pi_q=(a+p)/(a+q)` and `E_pi rho=p+2a(q-p)/(a+q)>p`.  Hence for every increasing convex radial observable `f`, `sum_i f(rho_i)T_i=D(E_pi f-f(p))>0` by Jensen; in particular every power moment `rho^alpha`, `alpha>=1`, increases.  This sign-definite class cannot be a hidden finite UV budget: every nonconstant increasing convex function on `[0,infinity)` is unbounded.
### Vector heat-sphere source + diamond leakage — DISTILLED EXACT / EXACT / EXTERNAL EXACT RIGIDITY
Fix a child wavevector `q` and one same-helicity parent sector `h`.  Write the unordered parent pair as `p=(q+r)/2`, `m=(q-r)/2`; then its physical heat rate is `kappa=(|q|^2+|r|^2)/2`, so every fixed-`kappa` fiber is the parent-separation sphere `S^2_(sqrt(2kappa-|q|^2))/(r~-r)=RP^2`.  The primitive object is the unordered Leray **vector** orbit coefficient `F_q(r)` in `q^perp`; the continuum source density is `C_F F_q(r)`, and minority/spin-two components are merely its two child-helicity coordinates.  With `P=|p|, M=|m|, Q=|q|`, exact Waleffe/Leray algebra gives `||F_q(r)||=|q.r||qxr| sqrt((P+M)^2+Q^2)|a_pa_m|/[4PMQ(P+M)]`.  Thus the complete geometric zero set on a nondegenerate fiber is exactly `q.r=0` (equal-radius Beltrami equator) union `qxr=0` (collinear shear pole).  For the vector-measure polar decomposition `dF=xi d sigma`, `||xi||=1`, Hilbert-space polarization gives `sigma(Omega)^2-||int dF||^2=(1/2)int int||xi(r)-xi(r')||^2 d sigma d sigma'`; every off-diagonal pair is an actual same-output Fourier diamond.  The geometric source line is explicit.  In the triad-normal frame `ell_q(r)=[t_q+i h beta n]`, `beta=Q/(P+M)`.  For two heat-fiber atoms with dihedral angle `psi`, `1-|<zhat,zhat'>|^2=[(beta-beta')^2 cos^2 psi+(1-beta beta')^2 sin^2 psi]/[(1+beta^2)(1+beta'^2)]`.  Hence equal projective source lines require `beta=beta'` and `psi=0 mod pi`.  On one heat sphere the generic nontrivial double fiber is exactly the equatorial reflection `r=u+Z qhat`, `r'=u-Z qhat`, a planar cross-isospectral rectangle.  Its two equal-radius cross pairs are Beltrami-null, but the other two outputs `q+-u` have common geometric source coefficient `G_side` satisfying `G_side/G_0=|Z|/sqrt(Q^2+U^2)*sqrt(((P+M)^2+Q^2+U^2)/((P+M)^2+Q^2))`; with arbitrary amplitudes `sqrt(sigma_side,+ sigma_side,-)=(G_side/G_0)sqrt(sigma_1 sigma_2)`.  Thus exact line-aligned two-atom cancellation leaks to side outputs unless the original atom itself approaches the Beltrami equator `Z=0`.  At measure level there is an exact projective disintegration, not an entropy.  Put `S=sigma(Omega)`, `lambda=ell_#sigma`, disintegrate `d sigma=d nu_ell d lambda` with conditional probabilities, and set `b(ell)=int xi d nu_ell`.  Then `S^2-||F||^2=S int(1-||b||^2)d lambda+(1/2)int int||b(ell)-b(ell')||^2d lambda d lambda'`; the first term is within-line phase/amplitude cancellation, while the second is between-line cancellation and is `>=(1/2)int int||b||||b'||D_proj(ell,ell')d lambda d lambda'`.  Generic within-line fibers are exactly the reflection rectangles already routed above.  More general off-diagonal diamonds retain the existing unequal-radius/nonparallel cross-source lower bound, and zero tolerance forbids all non-original pair-source contributions from vanishing simultaneously.  The missing measure bridge has an exact geometric base.  For two physical unordered same-output parent orbits and the `1/2` symmetry interchanging them, generic equal-heat incidence is `dXi=(1/8)dq dp dp' delta(kappa_q(p)-kappa_q(p'))`.  Since `kappa_q(p)-kappa_q(p')=2(p-p').(p+p'-q)`, coarea gives `dXi=dp dp' dH^2(y)/(16|p-p'|)`, `y perp(p-p')`, with the four physical modes `p,p',p'+y,p+y`; all four non-original pairings are retained.  Its typed quadratic target is also canonical: `S2(t)=int dz int|C_F S_pair|^2d lambda_z^unord<=2C_F^2E(t)||grad u||_2^2`, hence `int_0^T S2dt<=C_F^2E_*^2/nu`.  More strongly, disintegrating the unordered base by `kappa` gives exact fiber mass `lambda_(q,kappa)(RP^2)=pi R/4`.  If `S,F,L2` denote source variation/resultant/source-square on that fiber, Cauchy gives `4(S^2-||F||^2)/(pi R)<=L2`.  Let `P,N` be the actual positive/negative child-energy work obtained by projecting the same source atoms on the physical child vector `u_q`; pairwise Hilbert variance gives `S^2-||F||^2>=PN/|u_q|^2`, hence `4PN/(pi R|u_q|^2)<=L2`.  If `F=0`, then `P=N`: hidden work must meet actual opposite work and pay pair-source-square.  This closes the measure-level cancellation currency without identifying projective incidence with cross-source mass.  Coarea remains the exact geometry of where cross leakage lives; its plane/Radon multiplicity is optional unless one needs that downstream localization.  Independently, `B(v_h)=inf_K||(curl-hK)v_h||_2` and `S(v_h)=inf_e[int(1-(k.e)^2/|k|^2)|vhat_h|^2]^(1/2)` control every surviving modal source by `C_F||v_h||_2B` and `2C_FKmax||v_h||_2S`; both zero sets are exactly the same Beltrami/shear null manifolds.  Finally each source atom satisfies `(partial_t+nu kappa)f=C_q(N_pa_m+a_pN_m)`: absent actual parent forcing, cancellation persisting on an interval is Laplace-unique and must cancel fiberwise on the same `RP^2` heat sphere.  These are source/state laws, never a Hahn/work currency.  Kishimoto--Yoneda remains only the external zero-tolerance finite-mode endpoint.  The source-hiding grammar is therefore complete: net source survives as an actual child-state source (its energy projection is work), or persistent unforced hiding is fiberwise and pays `L2`, or actual parent nonlinear forcing is the retuning term.  The remaining seam is finite-time **accumulation**: the `S2` payment on a natural scale-`M` window may be only `O(M^-2)`, so this finite spacetime budget is not an event count.  No Hodge/entropy variable is primitive.
## 13. Frontier in one sentence
> **Deep minority creation is radial-variation controlled, and source hiding itself is now typed completely: survive as net Leray source, cancel on a physical heat sphere while paying canonical pair-source-square, or be actively retuned by actual parent nonlinearity.  The sole global seam is accumulation: those genuine payments are still parabolically summable across a geometric UV sequence, so a new NS-native non-summable rigidity is required.**
