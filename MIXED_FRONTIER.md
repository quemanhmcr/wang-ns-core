...
With signed radius `x=s|k|`, the cyclic work law gives `sum x_iT_i=0`, hence the canonical donor flow has zero signed `x` displacement.  Since a triad has at most two flows, any helicity-flip displacement `(rho_d+rho_r)M` is balanced by same-helicity radial displacement; globally `V_x<=2V_rho`.  Thus same-helicity bad outflow is ordinary radial layer-cake traffic and helicity-flip outflow is signed-radius layer-cake traffic already controlled by the same radial variation.  Bad recycling adds no third flow ledger; the unresolved endpoint is accumulation of the existing critical radial variation / `Hdot^(3/2)` activity itself.
...
1. **Critical-variation accumulation.**  Determine whether the hard-shell first-contact descent plus `V_x<=2V_rho<=2C_sh sqrt(mu_sh)H_(3/2)` forces a native time/scale obstruction to infinite UV contacts, without treating `H_(3/2)` as a pre-bounded budget.
2. **Endpoint sharpness.**  Stress-test explicit self-similar/discretely self-similar critical scenarios against the exact shell-collision and first-contact laws; any survivor identifies the genuinely missing NS structure.
## 13. Frontier in one sentence
> **Good first contacts already descend to lower hard interaction states; bad sidecar stock/outflow has no independent recycling ledger because signed-helicity boundary traffic is controlled by the same radial variation.  The sole remaining endpoint seam is accumulation of critical radial variation and `Hdot^(3/2)` activity itself.**
