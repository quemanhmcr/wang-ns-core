...
## 12. Pressure tidal + vorticity-frame closure
### Signed-curl source, same-field pressure quadrupole, and local dynamics — DISTILLED EXACT DEDUCTION
The signed-curl path/Betchov theorem remains the exact static spine: closed-triad work is the oriented path `alpha--beta--gamma`, radial traffic is its fold `rho=|x|`, `C_F int Z_tri dLambda_tri=2Q` with `Q=int omega.S omega`, Piola/Betchov gives `Q=-4int det S`, instantaneous opposite-sign `Z_tri` cancellation is real, and a finite singular endpoint forces `sup_(t<T*)int_0^tQ=+infinity`.  The pressure counterterm now has an equally exact type.  Put `g=-Delta p=|S|^2-|omega|^2/2`, `H0=Hess p+(g/3)I`.  For every `z!=0`, `H0hat(z)=(I/3-zhat tensor zhat)ghat(z)` and `ghat(z)=-C_F int(z.uhat(x))(z.uhat(z-x))dx`; an unordered pair contributes bare `-2(z.u_x)(z.u_y)` with helical geometric magnitude `|xxy|^2|a_xa_y|/(|x||y|)`.  Hence fixed-output pressure cancellation is only scalar cancellation on one tensor line; a deterministic two-pair Fourier state cancels `ghat(z)` exactly while retaining nonzero Leray source.  In physical space `H0=(4pi)^-1 PV int_0^infinity dr/r int_(S^2)(3n tensor n-I)g(x-rn)dOmega`, so only the `l=2` angular quadrupole of the **same NS scalar source** survives.  Poisson trace alone cannot close sign/orientation: `(4pi)^-1 int_(S^2)(3n tensor n-I)(n.Mn)dOmega=(2/5)M` for every STF `M`, so the pure elliptic map spans all tidal tensors; the missing restriction must use `g`'s same-field origin.  At `omega!=0`, with `xi=omega/|omega|`, `P=I-xi xi`, `a=xi.Sxi`, `b=P S xi`, `D=P S P+(a/2)P`, `tau=xi.H0xi`, exact NS gives `S=a xi xi+xi b+b xi-(a/2)P+D`, `|S|^2=3a^2/2+2|b|^2+|D|^2`, `D_t|omega|=a|omega|+nu xi.Deltaomega`, `D_txi=b+(nu/|omega|)P Deltaomega`, and `D_ta=-a^2/2+5|b|^2/3+|D|^2/3-|omega|^2/6-tau+nu[2b.Deltaomega/|omega|+xi.(Delta S)xi]`.  Pressure-Hessian ontology and local kinematics are therefore closed without a new owner/currency.
...
> **Pressure-typing frontier closure:** radial/helicity traffic is already the signed-curl path, and its integrated `Z_tri` field is the Betchov/enstrophy source.  Pressure adds no independent tensor mechanism: at each Fourier output it is one scalar longitudinal response, in physical space its trace-free part is exactly the `l=2` quadrupole of `g=|S|^2-|omega|^2/2`, and in the vorticity frame its effect on stretching rate is the single tidal scalar `tau=xi.H0xi`.  Beltrami-equator and shear zeros are now physically separated, and exact fixed-output pressure cancellation can coexist with surviving Leray source.  The elliptic operator alone is deliberately **not** enough—its `l=2` map spans every STF tidal tensor—so the next nonredundant theorem must exploit the same-field compatibility between the late first-contact `a,b,D,omega` geometry and the quadrupole source `g` generated by that very `u`.  Lamb, `J,Lambda,S2_edge,K2_edge`, Cauchy, coarea, pressure-pair entropy and `tau` remain readers/diagnostics, never owners or currencies.**
