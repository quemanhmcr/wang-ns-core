...
For the endpoint spine no extremizer or efficiency threshold is needed.  On a positive child edge `e=(x,y->z)`, let `xi_i=s_i|k_i|` be the three signed roots and `Z_e=sum_i xi_i^2T_i`.  Define the **exact geometric-good restriction** `G` to consist of those positive edges for which, after simultaneous reversal of all helicity signs if necessary and relabeling only the two non-child roots,
\[
oxed{-r<d<c=|z|,\qquad Z_e>0,\qquad rac35<rac dc,rac rc<rac58.}
\]
Define `B_bad` to be the complementary restriction of this same `dW+`.  This is a Borel partition determined by finitely many sign choices and strict algebraic inequalities in the actual triad variables:
\[
oxed{dW^+=dW^+|_G+dW^+|_{B_{bad}}.}
\]
The name `B_bad` means only “endpoint complement of the exact geometric-good set”; it is distinct from the later minority-helicity critical branch `B_crit`.  No auxiliary extremizer, efficiency coordinate, probability, owner or second Hahn enters this definition.
...
Generic triads can have two energy donors.  The geometric-good `G` case has one energy donor but can still have a positive nonforward side recipient; that side work is real energy transfer, not dissipation and not a reset.
...
\]
Therefore `[S_eta,tri]_+=alpha_tri[-T_p]_+`: fine positive symmetric strain work is a positive restriction of the **original canonical low-root `dW-`**, not a new Hahn law.  After any deterministic coarsening, `[pi#dS]_+<=pi#(alpha dW-)`; the cyclic donor kernel pushes that inherited restriction to existing canonical `dW+` at the same physical time.  Every such recipient lies outside the recursive geometric-good channel: if it is below the other unresolved root it is nonforward; otherwise its resolved interaction parent is `<1/2` of the child, contradicting the defining `G` lower bound `3/5`.  Thus symmetric interface work adds no recursive owner; `K` remains zero-depth conservative relink and this positive `S` pushforward lies in the complementary canonical `B_bad` restriction, where fixed-control-volume Kirchhoff—not a new source law—handles it.  Changing the resolved cutoff only repartitions the same `-Q B(u,u)`, so this quotient does not change the NS law.
...
A looser valid old-pool capacity bound can change `C_old>Y/8` without changing the field or realized service, so that threshold cannot define an event.  The historical old-pool half-life remains valid only under its separately supplied low-strain and directional scale-ratio hypotheses and is not a generic source clock.
...
### `5/8` geometric-good boundary-crossing theorem — EXACT DEDUCTION
...
For every geometric-good `G` atom whose recipient lies in `C_m`, the signed-root definition and the exact root-work identities give the unique same-time energy donor
\[
\frac35|k_c|<|k_d|<\frac58|k_c|\le R_{m+1}.
\]
Hence geometric-good work into `C_m` can never be internal traffic: it is actual nonlinear boundary inflow.  Since `|k_c|>R_(m+1)`, also `|k_d|>3R_m/8>R_(m+3)`, so donor support lies only in `C_(m+1)` or `C_(m+2)`.  Nonforward positive work leaves this carried-root recursion; the general non-`G` complement is handled by fixed-control-volume Kirchhoff, not by a transfer-loss semantics.
...
For the low ball `L_R={|k|<=R}`, exact continuity gives `E_<=R(t)+D_<=R+Phi_up=E_<=R(0)+Phi_down`; hence any geometric-good layer crossing `R` obeys `mu_G<=Phi_up<=E_<=R(0)+Phi_down`.  Since `Phi_down` is nonforward positive child-work into `|k|<=R`, continuum variation yields `Phi_down[0,t]<=4sqrt(2)C_F t E_*^(3/2)sqrt(4pi/5)R^(5/2)`, and, because `E_<=R(0)->0` for `u_0 in L^2(R^3)`, `Phi_up(R;[0,t])->0` as `R->0`, with `E_*=||u_0||_2^2`.  This is IR funding/extinction, not a budget that may be charged once per cut.
...
Recall the exact geometric restrictions `G,B_bad` of the one Hahn-positive law from §2.  The endpoint complement `B_bad` is not the final full-state `B`: reserve `B_crit` for the positive minority-helicity critical branch determined by the exact identity `sum_i rho_i T_i=2 rho_m T_m` on a heterochiral triad (and `sum_i rho_iT_i=0` on a homochiral triad).  Thus the `G/B_bad` split and the later `B_crit/S/V/O` frontier are different classifications of the same native work law.

For a geometric-good `G` atom the definition already permits us, after simultaneous helicity reversal if necessary, to order the signed roots as `alpha=-r<beta=d<gamma=c`, where `c` is the main positive recipient, `d` the unique donor and `r` the opposite-helicity side recipient.  The endpoint proof uses only the defining support `3/5<d/c,r/c<5/8` and the two exact triad invariants `sum T_i=0`, `sum x_iT_i=0`.  With `Z=sum x_i^2T_i>0`, these give exactly
...
No new Hahn split, optimizer or temporal ancestry enters these identities.  The `G/B_bad` partition is the direct geometric restriction of `dW+` in §2, and once on `G` the endpoint `X/Y -> S/V/O` argument uses only its defining support and these signed-root identities.

### First-contact good-birth fertility — DISTILLED EXACT DEDUCTION
For any fixed `P_M={M/2<|k|<=M}`, let `tau>0` be the first contact of the actual stock level `M E_M=mu0`.  This is a state locator, not a new cause.  Smoothness gives `dot E_M(tau)>=0`; differentiating the exact mode-set law gives `phi_in,M(tau)>=2nu||grad P_Mu||_2^2>=nu M mu0/2`.  Hence endpoint-complement `B_bad` inflow rate is `>=nu M mu0/4`, or `G` rate is.  In the latter case the exact geometric definition puts both interaction parents in `U_M={3M/10<|k|<=5M/8}`.  The direct sharp work bound on the whole bands, with `C_Y=3sqrt(3)/2`, gives `phi_G<=(15sqrt(pi)/8) M sqrt(mu0) mu_U`, `mu_U=M||P_Uu||_2^2`; thus `mu_U>=2nu sqrt(mu0)/(15sqrt(pi))`.  Covering `U_M` by the two hard shells at `5M/8,5M/16` yields one required interaction shell `R<=5M/8` with `R E_R(tau)>=nu sqrt(mu0)/(36sqrt(pi))`.  Put `mu_*=min(c_nu/2,nu^2/(5184pi))`: at a `mu_*` contact the `G` alternative therefore forces `R E_R(tau)>=2mu_*`.  Either `R E_R(0)>=mu_*`, or continuity gives a strictly earlier `mu_*` contact of that lower shell.  Repeating only the `G` alternative has finite depth because scales contract by `<=5/8` while every contacted shell has `R>=mu_*/E_*`.  Thus every such state-contact descent ends at initial stock or a genuine `B_bad`-rate alternative; no energy-token parent is asserted.
...
Restrict this **existing boundary inflow** by the exact geometric `G/B_bad` partition of the same one-Hahn law.  One branch has child-scale mass at least `2c_nu/5`.

If `B_bad` is the larger branch, split the original hard shell only by its two child helicities.  For one helicity cell `A` with `N_A=M`,
\[
N_A\Phi_{in,A}^{B_{bad}}\ge c_\nu/5,
\]
so direct Kirchhoff already gives one of `N_AE_A(t_1)`, `N_AD_A`, `N_A Phi_(out,A)` at least `c_nu/15`.

If `G` is the larger branch, its defining signed roots `-r<d<c`, `3/5<d/c,r/c<5/8` and the exact energy/signed-helicity identities give
\[
T_{-r}=\frac Z{(d+r)(c+r)},\quad T_d=-\frac Z{(d+r)(c-d)},\quad T_c=\frac Z{(c+r)(c-d)},
\]
so `W_side/W_child=(c-d)/(d+r)>3/10` and `M Phi_side>3c_nu/25`.  The side recipient has opposite helicity to the unique donor and lies in `(3M/10,5M/8]`.  Cover by `(M/4,M/2]` and `(M/2,M]` and both helicities.  If `q=max_A N_A Phi_(in,A)^side`, two cells have `N_A=M/2` and two `N_A=M`, so `M Phi_side<=2(2q)+2q=6q`; hence `q>=c_nu/50`.

In either branch select the corresponding fixed helicity cell and call the retained positive inflow restriction `Phi_in,A^sel`.  The preceding bounds give the common estimate `N_A Phi_(in,A)^sel>=c_nu/50` (and `>=c_nu/5` on the `B_bad` branch).  Exact Kirchhoff on that cell gives
\[
...
\]
Using the weaker common threshold `c_nu/150`, these are exactly `S vee V vee O`: stock, physical viscosity, or actual modal outflow; there is no fourth kinetic fate.
