...
### Critical endpoint rigidity + hard-shell gate — DISTILLED EXACT / EXTERNAL EXACT
Kenig--Koch, arXiv:0908.3349, Theorem 0.1 says bounded `Hdot^(1/2)` on the maximal mild lifespan implies global smoothness; thus finite `T_*` forces `C_(1/2)(t)=||u(t)||_Hdot^(1/2)^2` unbounded as `t->T_*`.  On every compact pre-singular interval `C_(1/2)` is absolutely continuous, so unboundedness forces times `t_n->T_*` with `dC_(1/2)/dt(t_n)>0`.  The hard-shell collision above then gives radii `M_n>0` with `M_n||P_(M_n/2,M_n]u(t_n)||_2^2>(2nu/C_sh)^2`.  Hence a hypothetical finite singularity must instantiate the same fixed critical hard-shell state arbitrarily late.  This is an endpoint gate, not a proof that `M_n->infinity` or that such states cannot accumulate.  The initial boundary is already exact modal stock at `t=0`.
...
The hard-state **kinetic-energy layer** is exact: earlier modal stock and canonical boundary inflow are the only suppliers; viscosity/outflow are losses.  Signed-good inflow has the `5/8` donor geometry; same-time donor reroot has zero depth; and the resolved `K/S` split adds no recursive owner—`K` is conservative relink while fine `S+` is an inherited low-root `dW-` restriction whose recipient pushforward is canonical bad work.  Pure-`G` funding is radial-cut rooted and total radial variation collides with actual hard-shell concentration `mu_sh`; no independent inward-reuse currency remains.  The initial traffic boundary is exact stock at `t=0`; finite singularity would force the fixed critical shell quantum arbitrarily late, and the sole unresolved global theorem is whether such hard-shell states can accumulate at a finite time.  Degenerate Young/Christ remains auxiliary.  See `MIXED_FRONTIER.md`.
