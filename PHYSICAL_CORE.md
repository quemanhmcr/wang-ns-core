...
### Critical radial first moment — DISTILLED EXACT; total-variation collision — EXACT
On every compact smooth slab, `C_(1/2)=int |k||uhat|^2=||u||_Hdot^(1/2)^2` obeys `C_(1/2)(t)+2nu D_(3/2)+A_down=C_(1/2)(0)+A_up`, `D_(3/2)=int_0^t||u||_Hdot^(3/2)^2`.  A signed-good triad has `9/25 < P_G/(rho_c W_c+) < 5/12`, hence `(9/25)int rho_c dW_G<=C_(1/2)(t)+2nu D_(3/2)+A_(down,notG)`.  The remaining inward term is not an independent currency: for one closed triad, `M_(d->r)<=min(|T_d|,|T_r|)`, `|T_i|<=4rho_i|a_0a_1a_2|`, and the triangle law give `|rho_r-rho_d|M_(d->r)<=4LK|a_0a_1a_2|`, `L=min rho_i`, `K=max rho_i`.  At most two donor flows occur.  Dyadic annuli are then only readers: the two high roots are comparable, the low-root Fourier `L1` bound gives a block factor `(L/K)^2`, and geometric summation yields the invariant estimate `v_rad(t):=int|rho_r-rho_d|dM_t <= C_rad||u(t)||_Hdot^(1/2)||u(t)||_Hdot^(3/2)^2` for one universal `C_rad<infinity`.  Therefore `A_(down,notG)<=C_rad int||u||_Hdot^(1/2)||u||_Hdot^(3/2)^2`, so good UV action can escape only through critical `Hdot^(1/2)` growth or critical `Hdot^(3/2)` activity; neither is a finite reset budget.
...
### Critical endpoint rigidity — EXACT / EXTERNAL EXACT
The radial ledger gives `d C_(1/2)/dt+2nu||u||_Hdot^(3/2)^2<=C_rad||u||_Hdot^(1/2)||u||_Hdot^(3/2)^2`; hence below `||u||_Hdot^(1/2)<=nu/C_rad`, viscosity absorbs all gross radial turnover and `C_(1/2)` cannot increase.  More generally, Kenig--Koch, arXiv:0908.3349, Theorem 0.1 proves that a mild Navier--Stokes solution with `u_0 in Hdot^(1/2)` which stays bounded in `Hdot^(1/2)` on its maximal lifespan is global and smooth.  Therefore a finite singular time `T_*` forces `limsup_(t->T_*)||u(t)||_Hdot^(1/2)=infinity`.  This identifies the only surviving singular endpoint channel; it does not bound that critical stock for arbitrary large data.  The initial boundary itself is already exact mode-set stock at `t=0`, not another ancestry interface.
...
The hard-state **kinetic-energy layer** is exact: earlier modal stock and canonical boundary inflow are the only suppliers; viscosity/outflow are losses.  Signed-good inflow has the `5/8` donor geometry; same-time donor reroot has zero depth; and the resolved `K/S` split adds no recursive owner—`K` is conservative relink while fine `S+` is an inherited low-root `dW-` restriction whose recipient pushforward is canonical bad work.  Pure-`G` funding is radial-cut rooted and total inward radial reuse is bounded by the critical `Hdot^(1/2)`–`Hdot^(3/2)` collision; no independent interior reuse currency remains.  The initial traffic boundary is exact stock at `t=0`; the sole unresolved global interface is whether the critical `Hdot^(1/2)` stock can become unbounded at a finite time.  Degenerate Young/Christ remains auxiliary.  See `MIXED_FRONTIER.md`.
